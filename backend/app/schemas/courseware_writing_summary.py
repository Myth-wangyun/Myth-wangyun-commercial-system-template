"""
Pydantic schemas for courseware writing summary
神殿智慧司课件编写汇总表 Pydantic 模式 - 支持动态列
"""
...
class CoursewareWritingSummaryCreate(BaseModel):
    """创建课件编写汇总请求"""
...
class CoursewareWritingSummaryUpdate(BaseModel):
    """更新课件编写汇总请求"""
...
class CoursewareWritingSummaryOut(BaseModel):
    """课件编写汇总响应"""
...
# 保留旧的 schema 以兼容旧接口
class CoursewareWritingRecordCreate(BaseModel):
    神殿名称: str
    年份: int
    网络新编: int = 0
    网络修改: int = 0
...
class CoursewareWritingRecordUpdate(BaseModel):
...
class CoursewareWritingRecordOut(BaseModel):
    id: int
    神殿名称: str
    年份: int
    网络新编: int
