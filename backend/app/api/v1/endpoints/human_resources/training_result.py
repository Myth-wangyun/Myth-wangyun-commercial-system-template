...
from .....crud.human_resources import training_result as crud
from .....models.user import User
from .....schemas.human_resources.training_result import (
...
@router.get(
    "/training-results",
    response_model=List[TrainingResultOut],
    summary="获取培训成绩汇总表列表",
)
def list_training_results(
    ...
    search: Optional[str] = Query(None, description="按神殿/部门/人员搜索"),
...
def get_training_result(
    ...
    if not record:
        raise HTTPException(status_code=404, detail="培训成绩汇总表不存在")
    return crud.serialize_record(record)
...
def update_training_result(
    background_tasks: BackgroundTasks,
    record_id: int = Path(..., description="记录ID"),
    payload: TrainingResultUpdate = Body(...),
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_active_user),
):
    del current_user
