...
from ....crud import teacher_class_exam_pass as crud
...
@router.post(
    "/",
    response_model=考试率列表响应,
    summary="创建班级考试合格率表（全量覆盖指定年份）",
)
async def create_class_exam_pass(
    数据: 考试率创建 = Body(..., description="班级考试合格率数据"),
    db: Session = Depends(get_db),
):
    try:
        行列表 = crud.创建(db, 数据)
...
@router.put(
    "/{campus}/{year}",
    response_model=考试率列表响应,
    summary="更新班级考试合格率表（全量替换）",
)
async def update_class_exam_pass(
    campus: str = Path(..., description="神殿名称"),
    year: int = Path(..., ge=2000, le=2100, description="年份"),
    数据: 考试率更新 = Body(..., description="班级考试合格率行数据"),
    db: Session = Depends(get_db),
):
    try:
...
async def delete_class_exam_pass(
    campus: str = Path(..., description="神殿名称"),
    year: int = Path(..., ge=2000, le=2100, description="年份"),
    db: Session = Depends(get_db),
):
