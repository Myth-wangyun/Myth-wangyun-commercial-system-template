...
from ....crud import teacher_project_pass as crud
...
@router.post(
    "/",
    response_model=项目率列表响应,
    summary="创建项目合格率表（全量覆盖指定年份）",
)
async def create_pass(
    数据: 项目率创建 = Body(..., description="项目合格率数据"),
    db: Session = Depends(get_db),
):
    try:
        行列表 = crud.创建(db, 数据)
...
@router.put(
    "/{campus}/{year}",
    response_model=项目率列表响应,
    summary="更新项目合格率表（全量替换）",
)
async def update_pass(
    campus: str = Path(..., description="神殿名称"),
    year: int = Path(..., ge=2000, le=2100, description="年份"),
    数据: 项目率更新 = Body(..., description="项目合格率行数据"),
    db: Session = Depends(get_db),
):
    try:
