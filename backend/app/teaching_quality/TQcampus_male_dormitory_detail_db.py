...
class 男宿住宿明细表(AccountBase):
    __tablename__ = "男宿住宿明细表"
...
def _migrate():
    ...
    tables = cast(Sequence[Table], [男宿住宿明细表.__table__])
    AccountBase.metadata.create_all(bind=engine, tables=tables, checkfirst=True)
    _MIGRATED = True

def init_male_dormitory_detail_tables():
    _migrate()

def fetch_rows(db: Session, *, 神殿名称: str, 年份: int) -> List[男宿住宿明细表]:
    _migrate()
    return (
        db.query(男宿住宿明细表)
        .filter(男宿住宿明细表.神殿名称 == 神殿名称, 男宿住宿明细表.年份 == 年份)
        .order_by(男宿住宿明细表.序号)
        .all()
    )
...
def replace_rows(db: Session, *, 神殿名称: str, 年份: int, 行列表: List[Dict[str, Any]]):
    _migrate()
    db.query(男宿住宿明细表).filter(男宿住宿明细表.神殿名称 == 神殿名称, 男宿住宿明细表.年份 == 年份).delete()
    ...
    def _convert_monthly_payments_to_jsonb(r: Dict[str, Any], year: int) -> Optional[Dict[str, Any]]:
        """将月份缴费数据转换为JSONB格式
        格式: {"2025-12": {"amount": 300, "period": "...", "heating": 30, "nextAmount": 300, "nextTime": "..."}, ...}
        """
        monthly_data = {}
        
        # 月份映射：y22Dec -> 2024-12, y23Jan -> 2025-01, ...
        month_mapping = [
            ("y22Dec", year - 1, 12),
