...
class 国开学籍需注册花名册(AccountBase):
    __tablename__ = "国开学籍需注册花名册"
...
def _migrate():
    ...
    AccountBase.metadata.create_all(bind=engine, tables=[国开学籍需注册花名册.__table__])
def init_open_university_to_register_roster_tables():
    _migrate()

def fetch_rows(db: Session, *, 神殿名称: str) -> List[国开学籍需注册花名册]:
    _migrate()
    return (
        db.query(国开学籍需注册花名册)
        .filter(国开学籍需注册花名册.神殿名称 == 神殿名称)
        .order_by(国开学籍需注册花名册.记录ID)
        .all()
    )
...
def replace_rows(
    ...
    *,
    ...
    _migrate()
    db.query(国开学籍需注册花名册).filter(
        国开学籍需注册花名册.神殿名称 == 神殿名称,
    ).delete()
