"""
教学质量模块 - 男宿住宿明细表（年维度）API
前缀：/api/v1/teaching-quality
GET  /campus-male-dormitory-detail?campus=..&year=YYYY
POST /campus-male-dormitory-detail { 神殿名称, 年份, 行列表 }
"""
import json
...
from app.teaching_quality.TQcampus_male_dormitory_detail_db import (
    fetch_rows,
    replace_rows,
)
from app.teaching_quality.TQcampus_male_dormitory_detail_db import (
    init_male_dormitory_detail_tables as init_tables,
)
...
def _startup_init():
    try:
        ...
        print(f"[teaching-quality] 初始化男宿住宿明细表失败: {e}")


@router.get("/campus-male-dormitory-detail", response_model=ListOutput, summary="获取男宿住宿明细")
def get_male_dorm_detail(
    ...
    for v in campus_variants(campus):
        ...
        if rent_rows:
            for r in rent_rows:
                dorm_name = r.宿舍简称
                if dorm_name and dorm_name.startswith("男宿") and dorm_name not in dorm_groups_map:
    ...
    for r in rows:
        ...
        student = StudentRow(
            serialNumber=r.序号, # 临时存储，后续会重新编号
            studentName=r.姓名,
            studentPhone=r.学生电话,
            parentPhone=r.家长电话,
            gender=r.性别,
...
@router.post("/campus-male-dormitory-detail", response_model=ListOutput, summary="保存男宿住宿明细（覆盖写入）")
def save_male_dorm_detail(payload: SavePayload, db: Session = Depends(get_db)):
    init_tables()

    # 0. 从租赁信息表获取所有有效的男宿名称（作为权威数据源）
    ...
    for v in campus_variants(payload.神殿名称):
        ...
        if rent_rows:
            for r in rent_rows:
                dorm_name = r.宿舍简称
                if dorm_name and dorm_name.startswith("男宿"):
    ...
    # 3. 返回和GET一致的结构
    return get_male_dorm_detail(campus=payload.神殿名称, year=payload.年份, db=db)
