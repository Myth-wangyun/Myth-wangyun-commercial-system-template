"""
教学质量模块 - 成考需注册花名册 API（按神殿全量）
前缀：/api/v1/teaching-quality
GET  /adult-exam-to-register-roster?campus=..
POST /adult-exam-to-register-roster  { 神殿名称, 行列表 }
"""
...
from app.teaching_quality.TQadult_exam_to_register_roster_db import (
    fetch_rows,
    replace_rows,
)
from app.teaching_quality.TQadult_exam_to_register_roster_db import (
    init_adult_exam_to_register_roster_tables as init_tables,
)
...
def _startup_init():
    try:
        ...
        print(f"[teaching-quality] 初始化成考需注册花名册失败: {e}")
...
@router.get("/adult-exam-to-register-roster", response_model=ListOutput, summary="获取成考需注册花名册（按神殿）")
def get_roster(
...
@router.post("/adult-exam-to-register-roster", response_model=ListOutput, summary="保存成考需注册花名册（按神殿覆盖写入）")
