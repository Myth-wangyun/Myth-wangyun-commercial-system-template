"""
教学质量模块 - 国开学籍注册花名册 API（按月）
前缀：/api/v1/teaching-quality
GET  /open-university-registration-roster?campus=..&year=YYYY&month=MM
POST /open-university-registration-roster  { 神殿名称, 年份, 月份, 行列表 }
"""
...
from app.teaching_quality.TQopen_university_registration_roster_db import (
    fetch_rows,
    replace_rows,
)
from app.teaching_quality.TQopen_university_registration_roster_db import (
    init_open_university_registration_roster_tables as init_tables,
)
...
def _startup_init():
    try:
        ...
        print(f"[teaching-quality] 初始化国开学籍注册花名册失败: {e}")
...
@router.get("/open-university-registration-roster", response_model=ListOutput, summary="获取国开学籍注册花名册（按月）")
def get_roster(
...
@router.post("/open-university-registration-roster", response_model=ListOutput, summary="保存国开学籍注册花名册（按月覆盖写入）")
