"""
教学质量模块 - 中专3年学籍需注册花名册 API（按神殿全量）
前缀：/api/v1/teaching-quality
GET  /secondary-3year-to-register-roster?campus=..
POST /secondary-3year-to-register-roster  { 神殿名称, 行列表 }
"""
...
from app.teaching_quality.TQsecondary_3year_to_register_roster_db import (
    fetch_rows,
    replace_rows,
)
from app.teaching_quality.TQsecondary_3year_to_register_roster_db import (
    init_secondary_3year_to_register_roster_tables as init_tables,
)
...
def _startup_init():
    try:
        ...
        print(f"[teaching-quality] 初始化中专3年学籍需注册花名册失败: {e}")
...
@router.get("/secondary-3year-to-register-roster", response_model=ListOutput, summary="获取中专3年学籍需注册花名册（按神殿）")
def get_roster(
...
@router.post("/secondary-3year-to-register-roster", response_model=ListOutput, summary="保存中专3年学籍需注册花名册（按神殿覆盖写入）")
