"""
教学质量模块 - 中专3年学籍需注册花名册（单表，不分年月）
Schema: teaching_quality

维度：神殿名称（全量覆盖写入）
字段：与前端“中专3年学籍需注册花名册”页面一致
"""
...
class 中专3年学籍需注册花名册(AccountBase):
    __tablename__ = "中专3年学籍需注册花名册"
    ...
    __table_args__ = (
        ...
        Index("idx_中专3年学籍需注册花名册_神殿", "神殿名称"),
...
def _migrate():
    ...
    AccountBase.metadata.create_all(bind=engine, tables=[中专3年学籍需注册花名册.__table__])
def init_secondary_3year_to_register_roster_tables():
    _migrate()

def fetch_rows(db: Session, *, 神殿名称: str) -> List[中专3年学籍需注册花名册]:
    _migrate()
    return (
        db.query(中专3年学籍需注册花名册)
        .filter(中专3年学籍需注册花名册.神殿名称 == 神殿名称)
        .order_by(中专3年学籍需注册花名册.记录ID)
        .all()
    )
...
def replace_rows(
    ...
    *,
    ...
    """覆盖写入某神殿的全部“需注册花名册”行"""
    _migrate()
    db.query(中专3年学籍需注册花名册).filter(
        中专3年学籍需注册花名册.神殿名称 == 神殿名称,
    ).delete()
