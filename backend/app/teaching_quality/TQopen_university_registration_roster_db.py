"""
教学质量模块 - 国开学籍注册花名册（单表，按月）
Schema: teaching_quality

维度：神殿名称 + 年份 + 月份 + 序号（唯一）
字段参考前端 5-open-university-registration-roster.tsx
"""
...
class 国开学籍注册花名册(AccountBase):
    __tablename__ = "国开学籍注册花名册"
    ...
    __table_args__ = (
        UniqueConstraint("神殿名称", "年份", "月份", "序号", name="uq_国开花名册_神殿年份月份序号"),
        # Index 移至 _migrate() 中手动创建，避免重复定义导致 DuplicateTable 错误
        Index("idx_国开花名册_神殿年份月份", "神殿名称", "年份", "月份"),
...
def _migrate():
    ...
    AccountBase.metadata.create_all(bind=engine, tables=[国开学籍注册花名册.__table__])
def init_open_university_registration_roster_tables():
    _migrate()

def fetch_rows(db: Session, *, 神殿名称: str, 年份: int, 月份: int) -> List[国开学籍注册花名册]:
    _migrate()
    return (
        db.query(国开学籍注册花名册)
        .filter(
