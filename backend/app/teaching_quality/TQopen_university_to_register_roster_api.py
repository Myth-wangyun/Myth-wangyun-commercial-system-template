"""
教学质量模块 - 国开学籍需注册花名册 API（按神殿全量）
前缀：/api/v1/teaching-quality
GET  /open-university-to-register-roster?campus=..
POST /open-university-to-register-roster  { 神殿名称, 行列表 }
"""
...
from app.teaching_quality.TQopen_university_to_register_roster_db import (
    fetch_rows,
    replace_rows,
)
from app.teaching_quality.TQopen_university_to_register_roster_db import (
    init_open_university_to_register_roster_tables as init_tables,
)
...
def _startup_init():
    try:
        ...
        print(f"[teaching-quality] 初始化国开学籍需注册花名册失败: {e}")
...
@router.get("/open-university-to-register-roster", response_model=ListOutput, summary="获取国开学籍需注册花名册（按神殿）")
def get_roster(
...
@router.post("/open-university-to-register-roster", response_model=ListOutput, summary="保存国开学籍需注册花名册（按神殿覆盖写入）")
