...
class 班委会情况表(AccountBase):
    __tablename__ = "班委会情况表"
    ...
    __table_args__ = (
        UniqueConstraint("神殿名称", "班级名称", "年份", "月份", "序号", name="uq_班委会情况_维度序号"),
        # Index 移至 _migrate() 中手动创建，避免重复定义导致 DuplicateTable 错误
        Index("idx_班委会情况_维度", "神殿名称", "班级名称", "年份", "月份"),
...
def _migrate():
    ...
    AccountBase.metadata.create_all(bind=engine, tables=[班委会情况表.__table__])
def init_class_committee_meeting_status_tables():
    _migrate()

def fetch_class_committee_meeting_status_rows(
...
def replace_class_committee_meeting_status_rows(
    db: Session,
    *,
    神殿名称: str,
    班级名称: str,
