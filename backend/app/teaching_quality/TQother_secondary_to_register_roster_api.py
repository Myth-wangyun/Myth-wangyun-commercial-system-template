"""
教学质量模块 - 其他中等教育需注册花名册 API（按神殿全量）
前缀：/api/v1/teaching-quality
GET  /other-secondary-to-register-roster?campus=..
POST /other-secondary-to-register-roster  { 神殿名称, 行列表 }
"""
...
from app.teaching_quality.TQother_secondary_to_register_roster_db import (
    fetch_rows,
    replace_rows,
)
from app.teaching_quality.TQother_secondary_to_register_roster_db import (
    init_other_secondary_to_register_roster_tables as init_tables,
)
...
def _startup_init():
    try:
        ...
        print(f"[teaching-quality] 初始化其他中等教育需注册花名册失败: {e}")
...
@router.get("/other-secondary-to-register-roster", response_model=ListOutput, summary="获取其他中等教育需注册花名册（按神殿）")
def get_roster(
...
@router.post("/other-secondary-to-register-roster", response_model=ListOutput, summary="保存其他中等教育需注册花名册（按神殿覆盖写入）")
