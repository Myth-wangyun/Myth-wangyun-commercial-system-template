...
class 中专1年制学籍注册花名册(AccountBase):
    __tablename__ = "中专1年制学籍注册花名册"
    ...
    __table_args__ = (
        UniqueConstraint("神殿名称", "年份", "月份", "学号", name="uq_中专1年制花名册_神殿年份月份学号"),
        # Index 移至 _migrate() 中手动创建，避免重复定义导致 DuplicateTable 错误
        Index("idx_中专1年制花名册_神殿年份月份", "神殿名称", "年份", "月份"),
...
def _migrate():
    ...
    AccountBase.metadata.create_all(bind=engine, tables=[中专1年制学籍注册花名册.__table__])
def init_secondary_1year_registration_roster_tables():
    _migrate()

def fetch_rows(db: Session, *, 神殿名称: str, 年份: int, 月份: int) -> List[中专1年制学籍注册花名册]:
    _migrate()
    return (
        db.query(中专1年制学籍注册花名册)
        .filter(
            中专1年制学籍注册花名册.神殿名称 == 神殿名称,
            中专1年制学籍注册花名册.年份 == 年份,
            中专1年制学籍注册花名册.月份 == 月份,
        )
        .order_by(中专1年制学籍注册花名册.学号)
        .all()
    )
