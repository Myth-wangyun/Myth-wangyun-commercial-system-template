...
class 其他中等教育需注册花名册(AccountBase):
    __tablename__ = "其他中等教育需注册花名册"
    ...
    __table_args__ = (
        ...
        Index("idx_其他中等教育需注册花名册_神殿", "神殿名称"),
...
def _migrate():
    ...
    AccountBase.metadata.create_all(bind=engine, tables=[其他中等教育需注册花名册.__table__])
def init_other_secondary_to_register_roster_tables():
    _migrate()

def fetch_rows(db: Session, *, 神殿名称: str) -> List[其他中等教育需注册花名册]:
    _migrate()
    return (
        db.query(其他中等教育需注册花名册)
        .filter(其他中等教育需注册花名册.神殿名称 == 神殿名称)
        .order_by(其他中等教育需注册花名册.记录ID)
        .all()
    )
...
def replace_rows(
    ...
    *,
    ...
    _migrate()
    db.query(其他中等教育需注册花名册).filter(
        其他中等教育需注册花名册.神殿名称 == 神殿名称,
    ).delete()
