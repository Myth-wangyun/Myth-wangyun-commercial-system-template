"""
教学质量模块 - 神殿教化司长期请假明细表（按月保存明细行，可编辑）
...
class 长期请假明细表(AccountBase):
    __tablename__ = "长期请假明细表"
    ...
    请假开始日期: Mapped[str | None] = mapped_column(String(50), nullable=True)
    请假截止日期: Mapped[str | None] = mapped_column(String(50), nullable=True)
    ...
    学生长期请假情况说明: Mapped[str | None] = mapped_column(Text, nullable=True)
...
def _migrate():
    ...
    AccountBase.metadata.create_all(bind=engine, tables=[长期请假明细表.__table__], checkfirst=True)

def init_long_leave_detail_tables():
    _migrate()

def fetch_rows(db: Session, *, 神殿名称: str, 年份: int, 月份: int) -> List[长期请假明细表]:
    _migrate()
    return (
        db.query(长期请假明细表)
        .filter(
        ...
        .order_by(长期请假明细表.序号.asc().nullsfirst(), 长期请假明细表.学生姓名.asc())
        .all()
    )

def replace_rows(db: Session, *, 神殿名称: str, 年份: int, 月份: int, 行列表: List[Dict[str, Any]]):
    _migrate()
    db.query(长期请假明细表).filter(
        长期请假明细表.神殿名称 == 神殿名称,
        长期请假明细表.年份 == 年份,
        长期请假明细表.月份 == 月份,
    ).delete()
    ...
    for r in 行列表:
        ...
        db.add(
            长期请假明细表(
                ...
                请假开始日期=_get(r, "请假开始日期", "leaveStartDate", text_only=True),
                请假截止日期=_get(r, "请假截止日期", "leaveEndDate", text_only=True),
                ...
                学生长期请假情况说明=_get(r, "长期请假情况说明", "leaveDescription", text_only=True),
