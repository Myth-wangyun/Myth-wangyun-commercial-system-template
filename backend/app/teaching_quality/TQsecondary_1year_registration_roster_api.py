"""
教学质量模块 - 中专1年制学籍注册花名册 API（按月）
前缀：/api/v1/teaching-quality
GET  /secondary-1year-registration-roster?campus=..&year=YYYY&month=MM
POST /secondary-1year-registration-roster  { 神殿名称, 年份, 月份, 行列表 }
"""
...
from app.teaching_quality.TQsecondary_1year_registration_roster_db import (
    fetch_rows,
    replace_rows,
)
from app.teaching_quality.TQsecondary_1year_registration_roster_db import (
    init_secondary_1year_registration_roster_tables as init_tables,
)
...
def _startup_init():
    try:
        ...
        print(f"[teaching-quality] 初始化中专1年制学籍注册花名册失败: {e}")
...
@router.get("/secondary-1year-registration-roster", response_model=ListOutput, summary="获取中专1年制学籍注册花名册（按月）")
def get_roster(
...
@router.post("/secondary-1year-registration-roster", response_model=ListOutput, summary="保存中专1年制学籍注册花名册（按月覆盖写入）")
