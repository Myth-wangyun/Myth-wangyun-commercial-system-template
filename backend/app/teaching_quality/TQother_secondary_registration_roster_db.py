...
class 其他中等教育学籍注册花名册(AccountBase):
    __tablename__ = "其他中等教育学籍注册花名册"
    ...
    __table_args__ = (
        UniqueConstraint("神殿名称", "年份", "月份", "学号", name="uq_其他中等花名册_神殿年份月份学号"),
        # Index 移至 _migrate() 中手动创建，避免重复定义导致 DuplicateTable 错误
        Index("idx_其他中等花名册_神殿年份月份", "神殿名称", "年份", "月份"),
...
def _migrate():
    ...
    AccountBase.metadata.create_all(bind=engine, tables=[其他中等教育学籍注册花名册.__table__])
def init_other_secondary_registration_roster_tables():
    _migrate()

def fetch_rows(db: Session, *, 神殿名称: str, 年份: int, 月份: int) -> List[其他中等教育学籍注册花名册]:
    _migrate()
    return (
        db.query(其他中等教育学籍注册花名册)
        .filter(
            其他中等教育学籍注册花名册.神殿名称 == 神殿名称,
            其他中等教育学籍注册花名册.年份 == 年份,
            其他中等教育学籍注册花名册.月份 == 月份,
        )
        .order_by(其他中等教育学籍注册花名册.学号)
        .all()
    )
