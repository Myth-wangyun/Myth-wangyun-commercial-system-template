"""
教学质量模块 - 中专3年学籍注册花名册（单表）
Schema: teaching_quality

查询维度：神殿名称 + 年份 + 月份（按月数据）
唯一约束建议：神殿名称 + 年份 + 月份 + 学号

字段映射（与前端 1-secondary-3year-registration-roster.tsx 对齐）：
- 学校名称 schoolName
...
- 户口性质 householdType
- 联系电话 contactPhone
- 户口所在地 householdAddress
- 招生对象 enrollmentTarget
- 是否随迁子女 isMigrantChild
- 注册年份 registrationYear（如 202409）
- 是否享受助学金 scholarshipStatus
- 家长姓名1 parentName1
- 家长1电话 parentPhone1
- 家长姓名2 parentName2
- 家长2电话 parentPhone2
...
class 中专3年学籍注册花名册(AccountBase):
    __tablename__ = "中专3年学籍注册花名册"
    ...
    学号: Mapped[str] = mapped_column(String(64), nullable=False)
    ...
    年级: Mapped[str | None] = mapped_column(String(20), nullable=True)
    ...
    __table_args__ = (
        UniqueConstraint("神殿名称", "年份", "月份", "学号", name="uq_中专3年花名册_神殿年份月份学号"),
        # Index 移至 _migrate() 中手动创建，避免重复定义导致 DuplicateTable 错误
        Index("idx_中专3年花名册_神殿年份月份", "神殿名称", "年份", "月份"),
...
def _migrate():
    ...
    AccountBase.metadata.create_all(bind=engine, tables=[中专3年学籍注册花名册.__table__])

def init_secondary_3year_registration_roster_tables():
    _migrate()

def fetch_rows(db: Session, *, 神殿名称: str, 年份: int, 月份: int) -> List[中专3年学籍注册花名册]:
    _migrate()
    return (
        db.query(中专3年学籍注册花名册)
        .filter(
            中专3年学籍注册花名册.神殿名称 == 神殿名称,
            中专3年学籍注册花名册.年份 == 年份,
            中专3年学籍注册花名册.月份 == 月份,
        )
        .order_by(中专3年学籍注册花名册.学号)
        .all()
    )
...
def replace_rows(
    ...
    *,
    ...
    """覆盖写入某神殿某年某月的中专3年学籍注册花名册。兼容中文键与英文键。"""
    _migrate()

    db.query(中专3年学籍注册花名册).filter(
        中专3年学籍注册花名册.神殿名称 == 神殿名称,
        中专3年学籍注册花名册.年份 == 年份,
        中专3年学籍注册花名册.月份 == 月份,
    ).delete()
    ...
    for r in 行列表:
        db.add(
            中专3年学籍注册花名册(
                ...
                年份=int(年份 or 0),
                月份=int(月份 or 0),
                学校名称=_get(r, "学校名称", "schoolName"),
                注册时间=_get(r, "注册时间", "registrationTime"),
                毕业时间=_get(r, "毕业时间", "graduationTime"),
                学号=str(_get(r, "学号", "studentNumber") or ""),
                姓名=_get(r, "姓名", "studentName"),
                性别=_get(r, "性别", "gender"),
                身份证号=_get(r, "身份证号", "idCardNumber"),
                专业=_get(r, "专业", "major"),
                年级=_get(r, "年级", "grade"),
                学制=_get(r, "学制", "educationSystem"),
                班级=_get(r, "班级", "className"),
                学习形式=_get(r, "学习形式", "studyMode"),
                民族=_get(r, "民族", "nation"),
