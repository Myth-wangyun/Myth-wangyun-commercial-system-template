"""
教学质量模块 - 其他中等教育学籍注册花名册 API（按月）
前缀：/api/v1/teaching-quality
GET  /other-secondary-registration-roster?campus=..&year=YYYY&month=MM
POST /other-secondary-registration-roster  { 神殿名称, 年份, 月份, 行列表 }
"""
...
from app.teaching_quality.TQother_secondary_registration_roster_db import (
    fetch_rows,
    replace_rows,
)
from app.teaching_quality.TQother_secondary_registration_roster_db import (
    init_other_secondary_registration_roster_tables as init_tables,
)
...
def _startup_init():
    try:
        ...
        print(f"[teaching-quality] 初始化其他中等教育学籍注册花名册失败: {e}")
...
@router.get("/other-secondary-registration-roster", response_model=ListOutput, summary="获取其他中等教育学籍注册花名册（按月）")
def get_roster(
...
@router.post("/other-secondary-registration-roster", response_model=ListOutput, summary="保存其他中等教育学籍注册花名册（按月覆盖写入）")
