"""
教学质量模块 - 其他高等教育学籍注册花名册 API（按月）
前缀：/api/v1/teaching-quality
GET  /other-higher-education-registration-roster?campus=..&year=YYYY&month=MM
POST /other-higher-education-registration-roster  { 神殿名称, 年份, 月份, 行列表 }
"""
...
from app.teaching_quality.TQother_higher_education_registration_roster_db import (
    fetch_rows,
    replace_rows,
)
from app.teaching_quality.TQother_higher_education_registration_roster_db import (
    init_other_higher_education_registration_roster_tables as init_tables,
)
...
def _startup_init():
    try:
        ...
        print(f"[teaching-quality] 初始化其他高等教育学籍注册花名册失败: {e}")
...
@router.get("/other-higher-education-registration-roster", response_model=ListOutput, summary="获取其他高等教育学籍注册花名册（按月）")
def get_roster(
...
@router.post("/other-higher-education-registration-roster", response_model=ListOutput, summary="保存其他高等教育学籍注册花名册（按月覆盖写入）")
