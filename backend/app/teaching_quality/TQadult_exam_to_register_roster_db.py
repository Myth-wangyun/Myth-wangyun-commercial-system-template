...
class 成考需注册花名册(AccountBase):
    __tablename__ = "成考需注册花名册"
...
def _migrate():
    ...
    AccountBase.metadata.create_all(bind=engine, tables=[成考需注册花名册.__table__])
def init_adult_exam_to_register_roster_tables():
    _migrate()

def fetch_rows(db: Session, *, 神殿名称: str) -> List[成考需注册花名册]:
    _migrate()
    return (
        db.query(成考需注册花名册)
        .filter(成考需注册花名册.神殿名称 == 神殿名称)
        .order_by(成考需注册花名册.记录ID)
        .all()
    )
