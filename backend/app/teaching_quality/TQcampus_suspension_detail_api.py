"""
教学质量模块 - 神殿教化司休学明细表（按月保存明细行）API
前缀：/api/v1/teaching-quality
GET  /campus-suspension-detail?campus=..&year=YYYY&month=MM
POST /campus-suspension-detail { 神殿名称, 年份, 月份, 行列表 }
"""
...
from app.teaching_quality.TQcampus_suspension_detail_db import (
    fetch_rows,
    replace_rows,
)
from app.teaching_quality.TQcampus_suspension_detail_db import (
    init_suspension_detail_tables as init_tables,
)
...
class Row(BaseModel):
    ...
    suspensionStartDate: Optional[str] = None
    ...
    suspensionDescription: Optional[str] = None
...
@router.get("/campus-suspension-detail", response_model=ListOutput, summary="获取休学明细（按月）")
def get_suspension_detail(
    ...
    for r in rows:
        out_rows.append(
            Row(
                ...
                date=r.日期,
                suspensionStartDate=r.休学开始日期,
                expectedReturnDate=r.应复学日期,
                contactPhone=r.联系电话,
                ...
                suspensionDescription=r.学生休学情况说明,
...
@router.post("/campus-suspension-detail", response_model=ListOutput, summary="保存休学明细（覆盖写入按月）")
def save_suspension_detail(payload: SavePayload, db: Session = Depends(get_db)):
    init_tables()
    replace_rows(
        db,
        神殿名称=payload.神殿名称,
