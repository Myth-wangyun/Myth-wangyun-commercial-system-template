"""
教学质量模块 - 中专1年制需注册花名册 API（按神殿全量）
前缀：/api/v1/teaching-quality
GET  /secondary-1year-to-register-roster?campus=..
POST /secondary-1year-to-register-roster  { 神殿名称, 行列表 }
"""
...
from app.teaching_quality.TQsecondary_1year_to_register_roster_db import (
    fetch_rows,
    replace_rows,
)
from app.teaching_quality.TQsecondary_1year_to_register_roster_db import (
    init_secondary_1year_to_register_roster_tables as init_tables,
)
...
def _startup_init():
    try:
        ...
        print(f"[teaching-quality] 初始化中专1年制需注册花名册失败: {e}")
...
@router.get("/secondary-1year-to-register-roster", response_model=ListOutput, summary="获取中专1年制需注册花名册（按神殿）")
def get_roster(
...
@router.post("/secondary-1year-to-register-roster", response_model=ListOutput, summary="保存中专1年制需注册花名册（按神殿覆盖写入）")
