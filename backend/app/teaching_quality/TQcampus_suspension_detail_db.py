...
class 休学明细表(AccountBase):
    ...
    休学开始日期: Mapped[str | None] = mapped_column(String(50), nullable=True)
    应复学日期: Mapped[str | None] = mapped_column(String(50), nullable=True)
    ...
    学生休学情况说明: Mapped[str | None] = mapped_column(Text, nullable=True)
...
def _migrate():
    ...
    AccountBase.metadata.create_all(bind=engine, tables=[休学明细表.__table__], checkfirst=True)

def init_suspension_detail_tables():
    _migrate()

def fetch_rows(db: Session, *, 神殿名称: str, 年份: int, 月份: int) -> List[休学明细表]:
    _migrate()
    return (
        db.query(休学明细表)
        .filter(
    ...
    for r in 行列表:
        ...
        db.add(
            休学明细表(
                ...
                日期=_get(r, "日期", "date", text_only=True),
                休学开始日期=_get(r, "休学开始日期", "suspensionStartDate", text_only=True),
                应复学日期=_get(r, "应复学日期", "expectedReturnDate", text_only=True),
                ...
                学生休学情况说明=_get(r, "学生休学情况说明", "suspensionDescription", text_only=True),
