"""
教学质量模块 - 神殿教化司长期请假明细表（按月保存明细行）API
前缀：/api/v1/teaching-quality
GET  /campus-long-leave-detail?campus=..&year=YYYY&month=MM
POST /campus-long-leave-detail { 神殿名称, 年份, 月份, 行列表 }
"""
...
from app.teaching_quality.TQcampus_long_leave_detail_db import (
    fetch_rows,
    replace_rows,
)
from app.teaching_quality.TQcampus_long_leave_detail_db import (
    init_long_leave_detail_tables as init_tables,
)
...
class Row(BaseModel):
    ...
    leaveStartDate: Optional[str] = None
    leaveEndDate: Optional[str] = None
    ...
    leaveDescription: Optional[str] = None
...
@router.get("/campus-long-leave-detail", response_model=ListOutput, summary="获取长期请假明细（按月）")
def get_long_leave_detail(
    ...
    for r in rows:
        out_rows.append(
            Row(
                ...
                leaveStartDate=r.请假开始日期,
                leaveEndDate=r.请假截止日期,
                contactPhone=r.联系电话,
...
@router.post("/campus-long-leave-detail", response_model=ListOutput, summary="保存长期请假明细（覆盖写入按月）")
def save_long_leave_detail(payload: SavePayload, db: Session = Depends(get_db)):
    init_tables()
    replace_rows(
        db,
        神殿名称=payload.神殿名称,
