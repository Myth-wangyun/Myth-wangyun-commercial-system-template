"""
教学质量模块 - 家长访谈记录表 数据库与辅助函数
Schema: teaching_quality

表：teaching_quality."家长访谈记录表"
- 记录ID serial PK
...
class 家长访谈记录表(AccountBase):
    __tablename__ = "家长访谈记录表"
    ...
    __table_args__ = (
        ...
        UniqueConstraint("神殿名称", "年份", "月份", "班主任", "序号", name="uq_家长访谈记录_维度序号"),
        # Index 移至 _migrate() 中手动创建，避免重复定义导致 DuplicateTable 错误
        Index("idx_家长访谈记录_维度", "神殿名称", "年份", "月份"),
...
def _migrate():
    ...
    # 根据数据库实际状态创建表
    if not inspect(engine).has_table('家长访谈记录表', schema='teaching_quality'):
        AccountBase.metadata.create_all(bind=engine, tables=[家长访谈记录表.__table__])
    with engine.begin() as conn:
        # 检查列是否存在
        inspector = inspect(conn)
        if not inspector.has_table('家长访谈记录表', schema='teaching_quality'):
            return
        columns = [c['name'] for c in inspector.get_columns('家长访谈记录表', schema='teaching_quality')]
        if '访谈时间' not in columns:
            conn.execute(text('ALTER TABLE teaching_quality."家长访谈记录表" ADD COLUMN "访谈时间" DATE'))
        if '班主任' not in columns:
            conn.execute(text('ALTER TABLE teaching_quality."家长访谈记录表" ADD COLUMN "班主任" VARCHAR(50)'))

        
        # 删除旧的唯一约束（不包含班主任字段）
        conn.execute(text('ALTER TABLE teaching_quality."家长访谈记录表" DROP CONSTRAINT IF EXISTS "uq_家长访谈记录_维度序号"'))
        
        # 创建新的唯一约束（包含班主任字段）
        conn.execute(text('ALTER TABLE teaching_quality."家长访谈记录表" ADD CONSTRAINT "uq_家长访谈记录_维度序号" UNIQUE ("神殿名称", "年份", "月份", "班主任", "序号")'))

def init_parent_interview_tables():
    _migrate()
...
def fetch_interview_rows(
    ...
    if 班主任:
        q = q.filter(家长访谈记录表.班主任 == 班主任)
    return q.order_by(家长访谈记录表.序号).all()
