...
class 教员功能分析学员违纪表(AccountBase):
    """存储学员违纪（行=教员，m1..m12=各月数据）"""

    __tablename__ = "教员功能分析学员违纪表"
    ...
    序号: Mapped[int] = mapped_column(Integer, nullable=False, comment="序号（教员序号）")
    ...
    __table_args__ = (
        Index("idx_违纪_神殿年份序号", "神殿名称", "年份", "序号", unique=True),
        {"schema": "academic"},
    )

    def __repr__(self):
        return f"<教员功能分析学员违纪表(神殿={self.神殿名称}, 年份={self.年份}, 序号={self.序号})>"
