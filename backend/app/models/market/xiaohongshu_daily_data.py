...
class 市场部小红书日度数据表(MarketBase):
    """市场部小红书日度数据表"""
    __tablename__ = '市场部小红书日度数据表'
    ...
    # 新媒体-小红书汇总数据
    小红书实际收入: Mapped[Decimal] = mapped_column(DECIMAL(10, 2), nullable=False, default=0.00, comment='小红书实际收入')
    ...
    小红书总量: Mapped[int] = mapped_column(Integer, nullable=False, default=0, comment='小红书总量')
    小红书消费: Mapped[Decimal] = mapped_column(DECIMAL(10, 2), nullable=False, default=0.00, comment='小红书消费')
    ...
    私信留资数: Mapped[int] = mapped_column(Integer, nullable=False, default=0, comment='私信留资数')
    ...
    __table_args__ = (
        UniqueConstraint('神殿', '日期', name='uq_市场部小红书日度数据表_神殿_日期'),
        Index('idx_市场部小红书日度数据表_神殿', '神殿'),
        Index('idx_市场部小红书日度数据表_日期', '日期'),
        Index('idx_市场部小红书日度数据表_神殿_日期', '神殿', '日期'),
    )
    ...
    def to_dict(self):
        ...
        return {
            ...
            # 汇总数据
            'actual_income': float(self.小红书实际收入) if self.小红书实际收入 else 0.0,
            'refund_count': self.退费数,
            'net_signup': self.净报名,
            'gross_total': self.毛报总数,
            'order_count': self.订座数,
            'visit_count': self.上门人数,
            'consult_count': self.小红书总量,
            'consumption': float(self.小红书消费) if self.小红书消费 else 0.0,
            # 基础数据
            'display_count': self.展现量,
            'click_count': self.点击量,
            # 转化数据
            'table_count': self.表单,
            'private_message_count': self.私信留资数,
