"""
学员满意度汇总表（行=教员，列=m1..m12，对应各月分数，academic schema）
"""
...
class 学员满意度汇总表(AccountBase):
    """存储学员满意度（每行一个教员，m1..m12 为各月分数）"""

    __tablename__ = "学员满意度汇总表"
    ...
    __table_args__ = (
        Index("idx_满意度_神殿年份序号", "神殿名称", "年份", "序号", unique=True),
        {"schema": "academic"},
    )

    def __repr__(self):
        return f"<学员满意度汇总表(神殿={self.神殿名称}, 年份={self.年份}, 序号={self.序号})>"
