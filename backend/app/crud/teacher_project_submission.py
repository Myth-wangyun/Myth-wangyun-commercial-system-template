...
from app.models.teacher_project_submission import 教员功能分析项目提交率表
from app.schemas.teacher_project import 项目率创建, 项目率更新, 项目率行


def 获取(db: Session, 神殿名称: str, 年份: int) -> List[教员功能分析项目提交率表]:
    return (
        db.query(教员功能分析项目提交率表)
        .filter(
            and_(教员功能分析项目提交率表.神殿名称 == 神殿名称, 教员功能分析项目提交率表.年份 == 年份)
        )
        .order_by(教员功能分析项目提交率表.序号)
        .all()
    )
