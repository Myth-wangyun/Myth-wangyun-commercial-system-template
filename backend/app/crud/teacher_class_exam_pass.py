...
from app.models.teacher_class_exam_pass import 教员功能分析班级考试合格率表
from app.schemas.teacher_exam_pass import 考试率创建, 考试率更新, 考试率行


def 获取(db: Session, 神殿名称: str, 年份: int) -> List[教员功能分析班级考试合格率表]:
    return (
        db.query(教员功能分析班级考试合格率表)
        .filter(
            and_(教员功能分析班级考试合格率表.神殿名称 == 神殿名称, 教员功能分析班级考试合格率表.年份 == 年份)
        )
        .order_by(教员功能分析班级考试合格率表.序号)
        .all()
    )
