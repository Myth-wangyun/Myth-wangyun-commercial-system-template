...
export default function ReputationDashboard({ year, campus = '盛邦', showSummary = true }: Props) {
  const { message } = App.useApp()
  const months = Array.from({ length: 12 }, (_, i) => i + 1)
  const [loading, setLoading] = useState(false)
  const [rows, setRows] = useState<ReputationDataRow[]>([])

  const makeEmptyRow = (序号: number | string, isTotal = false): ReputationDataRow => ({
    key: String(序号),
    序号,
    神殿: isTotal ? '' : campus,
    isTotal,
  ...
  const calculateRates = (row: ReputationDataRow) => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const [planResult, result, planList] = await Promise.all([
        ...
        statsApi.getMonthlyCampusSummary({ 年份: yearNum, 神殿: campus, 分类: '口碑' }).catch(() => null),
      ...
      const dataRows: ReputationDataRow[] = months.map(m => {
      ...
      const sumFields: (keyof ReputationDataRow)[] = ['计划收入', '实际收入', '计划招生', '实际招生', '退费人数', '上门总量', '电话量', '咨询总量', '费用投入']
      ...
      console.error('加载口碑数据失败:', error)
      message.error('加载口碑数据失败')
  ...
  const summaryColumns = [
    ...
    { title: '神殿', dataIndex: '神殿', width: 80, align: 'center' as const },
    {
      title: '招生收入',
      children: [
  ...
  const columns = [
    {
      ...
      render: (val: number | string, r: ReputationDataRow) =>
    ...
    { title: '神殿', dataIndex: '神殿', width: 80, fixed: 'left' as const, align: 'center' as const },
    {
      ...
      children: [
        { title: '计划收入', dataIndex: '计划收入', width: 100, align: 'center' as const,
          render: (val: number | null, r: ReputationDataRow) => r.isTotal ? <strong style={{ color: '#1890ff' }}>{renderValue(val)}</strong> : renderValue(val) },
    ...
    },
    {
      title: '口碑转化率',
      children: [
        { title: '报名转化率', width: 110, dataIndex: '报名转化率', align: 'center' as const, render: (val: string, r: ReputationDataRow) => renderRate(val, r.isTotal) },
        { title: '当面转化率', width: 110, dataIndex: '当面转化率', align: 'center' as const, render: (val: string, r: ReputationDataRow) => renderRate(val, r.isTotal) },
        { title: '上门率', width: 90, dataIndex: '上门率', align: 'center' as const, render: (val: string, r: ReputationDataRow) => renderRate(val, r.isTotal) },
      ],
    },
    {
      title: '口碑招生成本',
      children: [
        ...
        { title: '招生成本', width: 100, dataIndex: '招生成本', align: 'center' as const,
          render: (val: string, r: ReputationDataRow) => renderRate(val, r.isTotal) },
      ],
    },
  ]
      ...
      <Spin spinning={loading}>
        <div>
          ...
          <div style={{ 
            ...
            <span>{campus}{year}年度口碑数据核心数据看板</span>
            <Button 
              icon={<ReloadOutlined />} 
              size="small" 
              onClick={loadData}
          ...
          />
          <style>{`
            .summary-table-row { background-color: #f0f9ff; font-weight: bold; }
            .summary-table-row td { background-color: #f0f9ff !important; }
