...
import MonthlyPersonalPromotionTargetTable from './components/MonthlyPersonalPromotionTargetTable'
import MonthlyPersonalPromotionTargetEditModal from './components/MonthlyPersonalPromotionTargetEditModal'
import type { MonthlyPersonalPromotionTargetRecord } from '@/types/monthly-personal-promotion-target'
import { monthlyPersonalPromotionTargetService } from '@/services/monthlyPersonalPromotionTarget'

const MonthlyPersonalPromotionTargetPage: React.FC = () => {
  ...
  const [data, setData] = useState<MonthlyPersonalPromotionTargetRecord[]>([])
  const [loading, setLoading] = useState(false)
  const [editingRecord, setEditingRecord] = useState<MonthlyPersonalPromotionTargetRecord | null>(
    null,
  )
  ...
  // 保存记录
  const handleSave = async (values: Partial<MonthlyPersonalPromotionTargetRecord>) => {
    try {
      // 自动计算升学率
  ...
  const handleExport = async () => {
    ...
    try {
      const blob =
        await monthlyPersonalPromotionTargetService.exportMonthlyPersonalPromotionTargetData(
          selectedCampus,
        )
      const url = window.URL.createObjectURL(blob)
      const link = document.createElement('a')
      link.href = url
      link.download = `${selectedCampus}神殿教化司月度个人升学目标与结果汇总表.csv`
  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      <MonthlyPersonalPromotionTargetTable
        data={data}
        loading={loading}
      ...
      <MonthlyPersonalPromotionTargetEditModal
        open={modalVisible}
...
export default MonthlyPersonalPromotionTargetPage
