...
import type {
  ...
  CampusPersonalEnrollmentStatisticsTableProps,
} from '@/types/campus-personal-enrollment-statistics';
import './CampusPersonalEnrollmentStatisticsTable.css';

const CampusPersonalEnrollmentStatisticsTable: React.FC<CampusPersonalEnrollmentStatisticsTableProps> = ({
  dataSource,
  loading,
  onEdit,
  onDelete,
}) => {
  const columns: ColumnsType<CampusPersonalEnrollmentStatisticsRecord> = [
    {
      title: '序号',
      dataIndex: 'sequence',
      key: 'sequence',
      ...
      align: 'center',
      fixed: 'left',
      render: (value: number, record: CampusPersonalEnrollmentStatisticsRecord) => {
        if (record.isTotal) {
          return '';
        }
        return value;
    ...
    },
    {
      ...
      align: 'center',
      fixed: 'left',
      render: (value: string, record: CampusPersonalEnrollmentStatisticsRecord) => {
        if (record.isTotal) {
          return <Typography.Text strong style={{ color: '#cf1322' }}>合计</Typography.Text>;
        }
        return value;
      },
    },
    ...
    },
    {
      ...
      fixed: 'right',
      align: 'center',
      render: (_: any, record: CampusPersonalEnrollmentStatisticsRecord) => {
        if (record.isTotal) {
          return '';
        }
        return (
...
export default CampusPersonalEnrollmentStatisticsTable;
