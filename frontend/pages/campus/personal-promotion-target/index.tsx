...
import PersonalPromotionTargetTable from './components/PersonalPromotionTargetTable'
import PersonalPromotionTargetEditModal from './components/PersonalPromotionTargetEditModal'
import type { PersonalPromotionTargetRecord } from '@/types/personal-promotion-target'
import { personalPromotionTargetService } from '@/services/personalPromotionTarget'

const PersonalPromotionTargetPage: React.FC = () => {
  ...
  const [data, setData] = useState<PersonalPromotionTargetRecord[]>([])
  const [loading, setLoading] = useState(false)
  const [editingRecord, setEditingRecord] = useState<PersonalPromotionTargetRecord | null>(null)
  ...
  const fetchData = async (campus: string) => {
    ...
    setLoading(true)
    try {
      const result = await personalPromotionTargetService.getPersonalPromotionTargetData(campus)
      setData(result)
    } catch (error) {
  ...
  // 编辑记录
  const handleEdit = (record: PersonalPromotionTargetRecord) => {
  ...
  // 删除记录
  const handleDelete = (record: PersonalPromotionTargetRecord) => {
    if (record.rowType === 'total') {
  ...
  // 保存记录
  const handleSave = async (values: Partial<PersonalPromotionTargetRecord>) => {
    try {
      // 自动计算升学率
  ...
  const handleExport = async () => {
    ...
    try {
      const blob =
        await personalPromotionTargetService.exportPersonalPromotionTargetData(selectedCampus)
      const url = window.URL.createObjectURL(blob)
      const link = document.createElement('a')
      link.href = url
      link.download = `${selectedCampus}神殿教化司个人升学目标与结果汇总表.csv`
  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      <PersonalPromotionTargetEditModal
        open={modalVisible}
...
export default PersonalPromotionTargetPage
