// 学术->最高议事厅 课件编写汇总表主页面 - 支持动态列
import React, { useMemo, useState, useEffect, useCallback } from 'react';
import { App, Card, Table, Typography, Space, Button, InputNumber, Tag, Select, Input, Modal, Spin, Popconfirm } from 'antd';
import { DownloadOutlined, ReloadOutlined, FileTextOutlined, PlusOutlined, SaveOutlined, DeleteOutlined } from '@ant-design/icons';
...
const CoursewareWritingPage: React.FC = () => {
  ...
  const [data, setData] = useState<CoursewareWritingRecord[]>([]);
  ...
  const loadData = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await api.get(`/courseware-writing-summary/by-year/${year}`);
      ...
      // 转换为表格数据格式
      const tableData: CoursewareWritingRecord[] = campusNames.map((campus, index) => {
        ...
        // 构建记录（显示时去掉"神殿"后缀）
        const record: CoursewareWritingRecord = {
          id: `campus-${index + 1}`,
  ...
  const saveData = async () => {
    ...
    try {
      ...
      await api.post('/courseware-writing-summary/', {
        年份: year,
  ...
  // 计算每行的合计
  const computeTotals = (record: CoursewareWritingRecord) => {
    let totalNew = 0;
    let totalEdit = 0;
    dynamicColumns.forEach(col => {
  ...
  const columnTotals = useMemo(() => {
    ...
    totals.total_new = Object.keys(totals).filter(k => k.endsWith('_new')).reduce((sum, k) => sum + totals[k], 0) / dynamicColumns.length || 0;
    totals.total_edit = Object.keys(totals).filter(k => k.endsWith('_edit')).reduce((sum, k) => sum + totals[k], 0) / dynamicColumns.length || 0;
    // 重新计算正确的总计
  ...
  // 动态生成表格列
  const tableColumns: ColumnsType<CoursewareWritingRecord> = useMemo(() => {
    const cols: ColumnsType<CoursewareWritingRecord> = [
      {
        title: '序号',
  ...
  const exportCsv = () => {
    ...
    a.download = `最高议事厅智慧司课件编写汇总表_${year}_${new Date().toISOString().split('T')[0]}.csv`;
    a.click();
  };
    ...
    <Spin spinning={loading}>
      <div style={{ padding: 24 }}>
        ...
        <div className="mb-4">
          ...
          <Text type="secondary">统计所有神殿不同方向的新编与修改数量（支持动态添加列）</Text>
        </div>
