...
const CoursewareWritingPage: React.FC = () => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      console.error('[课件编写汇总] 加载数据失败:', error);
      // 使用默认空数据
  ...
  const saveData = async () => {
    ...
    try {
      ...
      console.error('[课件编写汇总] 保存失败:', error);
      message.error('保存失败');
    } finally {
      setSaving(false);
    }
        ...
        children: [
          {
            ...
            width: 100,
            align: 'center' as const,
            render: (_: unknown, r: CoursewareWritingRecord) => (
              <InputNumber
                min={0}
      ...
      children: [
        {
          ...
          width: 80,
          align: 'center' as const,
          render: (_: unknown, r: CoursewareWritingRecord) => <Tag color="blue">{computeTotals(r).totalNew}</Tag>,
          onHeaderCell: () => ({ style: { backgroundColor: '#e6f7ff' } }),
        },
        {
          title: '修改',
          ...
          width: 80,
          align: 'center' as const,
          render: (_: unknown, r: CoursewareWritingRecord) => <Tag color="geekblue">{computeTotals(r).totalEdit}</Tag>,
          onHeaderCell: () => ({ style: { backgroundColor: '#e6f7ff' } }),
        },
      ],
      onHeaderCell: () => ({ style: { backgroundColor: '#fffbe6' } }),
  ...
  const exportCsv = () => {
    ...
    a.download = `课件编写汇总_${campusLabel}_${new Date().toISOString().split('T')[0]}.csv`;
    a.click();
  };
  ...
  return (
    <Spin spinning={loading}>
      <div style={{ padding: 24 }}>
        ...
        `}</style>
        <div className="mb-4">
          <Title level={2}>11. {campusLabel}智慧司课件编写汇总表格</Title>
          <Text type="secondary">统计不同方向课件的新编与修改数量（支持动态添加列）</Text>
        </div>
