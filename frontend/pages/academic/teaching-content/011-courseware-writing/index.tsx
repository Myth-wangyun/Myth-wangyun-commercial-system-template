...
const CoursewareWritingPage: React.FC = () => {
  ...
  React.useEffect(() => {
    const loaded = loadCampusData<CoursewareWritingRecord[]>(
      STORAGE_KEYS.COURSEWARE_WRITING,
      currentCampus,
      data,
    )
    setData(loaded)
  ...
  React.useEffect(() => {
    saveCampusData(STORAGE_KEYS.COURSEWARE_WRITING, currentCampus, data)
  }, [data, currentCampus])

  const recompute = (r: CoursewareWritingRecord) => ({
    ...r,
    total_new: r.net_new + r.ai_new + r.aigc_new + r.media_new,
    total_edit: r.net_edit + r.ai_edit + r.aigc_edit + r.media_edit,
  })

  const onChange = (id: string, key: keyof CoursewareWritingRecord, v: number | null) => {
    setData((prev) =>
      prev.map((x) =>
        x.id === id ? recompute({ ...x, [key]: Number(v || 0) } as CoursewareWritingRecord) : x,
      ),
    )
  }

  const columns: ColumnsType<CoursewareWritingRecord> = [
  ...
  const exportCsv = () => {
    ...
    a.download = `课件编写汇总_${new Date().toISOString().split('T')[0]}.csv`
    a.click()
  }
  ...
  return (
    <div style={{ padding: 24 }}>
      <div className="mb-4">
        ...
        <Text type="secondary">统计不同方向的新编与修改数量</Text>
      </div>
      <div className="mb-4">
        <CampusSelector />
      </div>
