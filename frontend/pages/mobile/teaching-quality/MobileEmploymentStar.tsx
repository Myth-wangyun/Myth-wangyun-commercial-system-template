...
export default function MobileEmploymentStar() {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const res: any = await apiService.get('/teaching-quality/qt-employment-stars', {
              ...
              <div className="m-data-card" key={i}>
                ...
                <div className="m-data-row">
                  ...
                  <span className="m-data-value" style={{ color: '#52c41a', fontWeight: 600 }}>
                    ¥{(item.salary || item.薪资 || 0).toLocaleString()}
                  </span>
                </div>
              </div>
            ))}
