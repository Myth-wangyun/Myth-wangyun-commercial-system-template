...
export default function MobileAcademicCoreSummary() {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const res = await request.get('/academic-core-summary/', {
        params: { year },
      });
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                  <div>
                    ...
                    <div style={{ fontWeight: 600, color: '#52c41a' }}>
                      {item.employment_rate || item.就业率 || '-'}
                    </div>
                  </div>
                  <div>
                    <div style={{ color: '#999', fontSize: 12 }}>平均薪资</div>
                    <div style={{ fontWeight: 600 }}>
                      {item.avg_salary || item.平均薪资 || '-'}
                    </div>
                  </div>
                  <div>
                    <div style={{ color: '#999', fontSize: 12 }}>在校人数</div>
                    <div style={{ fontWeight: 600 }}>
                      {item.student_count || item.在校人数 || 0}
                    </div>
                  </div>
                  <div>
