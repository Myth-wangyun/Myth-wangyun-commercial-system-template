...
export default function MobileKpiPlan() {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const res = await api.get('/kpi-results', {
          ...
          <div style={{ padding: '0 12px' }}>
            {data.map((item: any, i: number) => (
              <div className="m-data-card" key={i}>
                ...
                <div className="m-data-row">
                  <span className="m-data-label">KPI项</span>
                  <span className="m-data-value">{item.kpi_name || item.项目 || '-'}</span>
                </div>
                <div className="m-data-row">
                  <span className="m-data-label">目标</span>
                  <span className="m-data-value">{item.target || item.目标 || '-'}</span>
                </div>
                <div className="m-data-row">
                ...
                <div className="m-data-row">
                  <span className="m-data-label">得分</span>
