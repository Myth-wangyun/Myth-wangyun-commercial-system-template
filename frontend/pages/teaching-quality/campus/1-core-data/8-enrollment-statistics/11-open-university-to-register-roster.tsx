...
interface OpenUniversityToRegisterRecord {
  key: string
...
const OpenUniversityToRegisterRoster: React.FC = () => {
  const { message, modal } = App.useApp()
  const { currentCampus } = useCampusStore()
  const [dataSource, setDataSource] = useState<OpenUniversityToRegisterRecord[]>([])
  const [isModalVisible, setIsModalVisible] = useState(false)
  const [editingRecord, setEditingRecord] = useState<OpenUniversityToRegisterRecord | null>(null)
  const [form] = Form.useForm<OpenUniversityToRegisterRecord>()
  ...
  const { fetchFromClassFile, loading: classFileLoading } = useClassFileRoster({
    category: 'open-university-to-register',
  ...
  const handleEdit = (record: OpenUniversityToRegisterRecord) => {
  ...
  const handleSaveModal = async () => {
    try {
      ...
      const newRecord: OpenUniversityToRegisterRecord = {
        ...editingRecord,
        ...formatted,
  ...
  const fetchFromServer = async () => {
    ...
    try {
      const res = await fetch(
        buildApiUrl(`/teaching-quality/open-university-to-register-roster?campus=${encodeURIComponent(currentCampus!)}`),
      )
      if (!res.ok) throw new Error(await res.text())
      const data = await res.json()
      const rows = (data?.行列表 || []) as any[]
      const mapped: OpenUniversityToRegisterRecord[] = rows.map((r: any, i: number) => ({
        key: `${i}-${r.idCardNumber || r.studentName || ''}`,
        pendingRegistrationTime: r.pendingRegistrationTime || '',
        studentName: r.studentName || '',
        gender: r.gender || '',
  ...
  const saveToServer = async () => {
    ...
    try {
      ...
      const res = await fetch(buildApiUrl('/teaching-quality/open-university-to-register-roster'), {
        method: 'POST',
  ...
  const columns: ColumnsType<OpenUniversityToRegisterRecord> = [
    { title: '需注册时间', dataIndex: 'pendingRegistrationTime', key: 'pendingRegistrationTime', width: 140, fixed: 'left' },
    { title: '姓名', dataIndex: 'studentName', key: 'studentName', width: 100, fixed: 'left' },
    { title: '性别', dataIndex: 'gender', key: 'gender', width: 70 },
    { title: '身份证件号', dataIndex: 'idCardNumber', key: 'idCardNumber', width: 190 },
    { title: '专业', dataIndex: 'major', key: 'major', width: 140 },
    { title: '学制', dataIndex: 'educationSystem', key: 'educationSystem', width: 100 },
    { title: '班级', dataIndex: 'className', key: 'className', width: 120 },
    { title: '民族', dataIndex: 'nation', key: 'nation', width: 100 },
    { title: '政治面貌', dataIndex: 'politicalStatus', key: 'politicalStatus', width: 120 },
    { title: '户口性质', dataIndex: 'householdType', key: 'householdType', width: 120 },
    { title: '联系电话', dataIndex: 'contactPhone', key: 'contactPhone', width: 140 },
    { title: '户口所在地', dataIndex: 'householdAddress', key: 'householdAddress', width: 220 },
    { title: '招生对象', dataIndex: 'enrollmentTarget', key: 'enrollmentTarget', width: 160 },
    { title: '是否随迁子女', dataIndex: 'isMigrantChild', key: 'isMigrantChild', width: 130 },
    { title: '注册年份', dataIndex: 'registrationYear', key: 'registrationYear', width: 120 },
    { title: '是否享受助学金（曾经/现在/即将享受）', dataIndex: 'scholarshipStatus', key: 'scholarshipStatus', width: 260 },
    { title: '家长姓名1', dataIndex: 'parentName1', key: 'parentName1', width: 120 },
    { title: '家长1电话', dataIndex: 'parentPhone1', key: 'parentPhone1', width: 140 },
    { title: '家长姓名2', dataIndex: 'parentName2', key: 'parentName2', width: 120 },
    { title: '家长2电话', dataIndex: 'parentPhone2', key: 'parentPhone2', width: 140 },
    { title: '神殿归属', dataIndex: 'campus', key: 'campus', width: 100 },
  ...
  return (
    <div style={{ padding: 24 }}>
      <Card
        ...
        extra={
          <Space>
            ...
            <Button type="dashed" icon={<PlusOutlined />} onClick={handleAdd}>新增记录</Button>
          </Space>
        }
      >
        <Table<OpenUniversityToRegisterRecord>
          bordered size="small" columns={columns} dataSource={dataSource}
          rowKey="key" pagination={false} scroll={{ x: 'max-content' }}
        />
      </Card>
      ...
      <Modal
        ...
        width={900}
      >
        <Form<OpenUniversityToRegisterRecord> form={form} layout="vertical">
          ...
          <Form.Item name="householdAddress" label="户口所在地"><Input /></Form.Item>

          <Space size="large" style={{ display: 'flex' }}>
            <Form.Item name="enrollmentTarget" label="招生对象"><Input placeholder="如：应届初中毕业生" /></Form.Item>
            ...
            <Form.Item name="registrationYear" label="注册年份"><Input placeholder="如：202409" /></Form.Item>
          </Space>

          <Form.Item name="scholarshipStatus" label="是否享受助学金（曾经/现在/即将享受）"><Input /></Form.Item>

          <Space size="large" style={{ display: 'flex' }}>
            <Form.Item name="parentName1" label="家长姓名1"><Input /></Form.Item>
            <Form.Item name="parentPhone1" label="家长1电话"><Input /></Form.Item>
          </Space>

          <Space size="large" style={{ display: 'flex' }}>
            <Form.Item name="parentName2" label="家长姓名2"><Input /></Form.Item>
            <Form.Item name="parentPhone2" label="家长2电话"><Input /></Form.Item>
          </Space>

          <Space size="large" style={{ display: 'flex' }}>
            <Form.Item name="campus" label="神殿归属"><Input /></Form.Item>
            <Form.Item name="headTeacher" label="班主任"><HomeroomTeacherSelect campusName={currentCampus || undefined} /></Form.Item>
          </Space>
        </Form>
      </Modal>
    </div>
...
export default OpenUniversityToRegisterRoster
