...
interface Secondary3YearToRegisterRecord {
  key: string
...
const Secondary3YearToRegisterRoster: React.FC = () => {
  const { message, modal } = App.useApp()
  const { currentCampus } = useCampusStore()
  const [dataSource, setDataSource] = useState<Secondary3YearToRegisterRecord[]>([])
  const [isModalVisible, setIsModalVisible] = useState(false)
  const [editingRecord, setEditingRecord] = useState<Secondary3YearToRegisterRecord | null>(null)
  const [form] = Form.useForm<Secondary3YearToRegisterRecord>()
  ...
  const handleEdit = (record: Secondary3YearToRegisterRecord) => {
  ...
  const handleSaveModal = async () => {
    try {
      ...
      const newRecord: Secondary3YearToRegisterRecord = {
        ...editingRecord,
        ...formatted,
  ...
  const fetchFromServer = async () => {
    ...
    try {
      const res = await fetch(
        buildApiUrl(`/teaching-quality/secondary-3year-to-register-roster?campus=${encodeURIComponent(currentCampus!)}`),
      )
      if (!res.ok) throw new Error(await res.text())
      const data = await res.json()
      const rows = (data?.行列表 || []) as any[]
      const mapped: Secondary3YearToRegisterRecord[] = rows.map((r: any, i: number) => ({
  ...
  const saveToServer = async () => {
    ...
    try {
      ...
      const res = await fetch(buildApiUrl('/teaching-quality/secondary-3year-to-register-roster'), {
        method: 'POST',
  ...
  const columns: ColumnsType<Secondary3YearToRegisterRecord> = [
    {
      title: '需注册时间',
  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      >
        <Table<Secondary3YearToRegisterRecord>
          bordered
          size="small"
          columns={columns}
      ...
      <Modal
        ...
        width={900}
      >
        <Form<Secondary3YearToRegisterRecord> form={form} layout="vertical">
...
export default Secondary3YearToRegisterRoster
