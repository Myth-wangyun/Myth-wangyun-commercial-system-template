...
interface OtherHigherEducationToRegisterRecord {
  key: string
...
const OtherHigherEducationToRegisterRoster: React.FC = () => {
  const { message, modal } = App.useApp()
  const { currentCampus } = useCampusStore()
  const [dataSource, setDataSource] = useState<OtherHigherEducationToRegisterRecord[]>([])
  const [isModalVisible, setIsModalVisible] = useState(false)
  const [editingRecord, setEditingRecord] = useState<OtherHigherEducationToRegisterRecord | null>(
    null,
  )
  const [form] = Form.useForm<OtherHigherEducationToRegisterRecord>()
  ...
  const { fetchFromClassFile, loading: classFileLoading } = useClassFileRoster({
    category: 'other-higher-to-register',
  ...
  const handleEdit = (record: OtherHigherEducationToRegisterRecord) => {
  ...
  const handleSaveModal = async () => {
    try {
      ...
      const newRecord: OtherHigherEducationToRegisterRecord = {
        ...editingRecord,
        ...formatted,
        key: editingRecord?.key || Date.now().toString(),
  ...
  const fetchFromServer = async () => {
    ...
    try {
      const res = await fetch(
        buildApiUrl(`/teaching-quality/other-higher-education-to-register-roster?campus=${encodeURIComponent(currentCampus!)}`),
      )
      if (!res.ok) throw new Error(await res.text())
      const data = await res.json()
      const rows = (data?.行列表 || []) as any[]
      const mapped: OtherHigherEducationToRegisterRecord[] = rows.map((r: any, i: number) => ({
  ...
  const saveToServer = async () => {
    ...
    try {
      ...
      const res = await fetch(buildApiUrl('/teaching-quality/other-higher-education-to-register-roster'), {
        method: 'POST',
  ...
  const columns: ColumnsType<OtherHigherEducationToRegisterRecord> = [
    {
      title: '需注册时间',
  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      >
        <Table<OtherHigherEducationToRegisterRecord>
      ...
      <Modal
        ...
        width={900}
      >
        <Form<OtherHigherEducationToRegisterRecord> form={form} layout="vertical">
...
export default OtherHigherEducationToRegisterRoster
