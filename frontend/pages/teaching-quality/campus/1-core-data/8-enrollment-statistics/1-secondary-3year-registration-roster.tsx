...
interface Secondary3YearRegistrationRecord {
  key?: string
  schoolName: string
...
const Secondary3YearRegistrationRoster: React.FC = () => {
  const { message, modal } = App.useApp()
  const { currentCampus } = useCampusStore()
  const [year, setYear] = useState<number>(new Date().getFullYear())
  const [month, setMonth] = useState<number>(new Date().getMonth() + 1)

  const [dataSource, setDataSource] = useState<Secondary3YearRegistrationRecord[]>([])
  const [isModalVisible, setIsModalVisible] = useState(false)
  const [editingRecord, setEditingRecord] = useState<Secondary3YearRegistrationRecord | null>(null)
  const [form] = Form.useForm<Secondary3YearRegistrationRecord>()
  ...
  const handleEdit = (record: Secondary3YearRegistrationRecord) => {
  ...
  const handleSaveModal = async () => {
    try {
      ...
      const formatted = {
        ...
        registrationYear: formatPickerValue(values.registrationYear, 'YYYYMM'),
      }
      const newRecord: Secondary3YearRegistrationRecord = {
        ...editingRecord,
        ...formatted,
  ...
  const fetchFromServer = async () => {
    ...
    try {
      const res = await fetch(
        buildApiUrl(`/teaching-quality/secondary-3year-registration-roster?campus=${encodeURIComponent(
          currentCampus!,
      ...
      const mapped: Secondary3YearRegistrationRecord[] = rows.map((r: any) => ({
  ...
  const saveToServer = async () => {
    ...
    try {
      ...
      const res = await fetch(buildApiUrl('/teaching-quality/secondary-3year-registration-roster'), {
        method: 'POST',
  ...
  const columns: ColumnsType<Secondary3YearRegistrationRecord> = [
    {
      title: '学校名称',
  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      >
        <Table<Secondary3YearRegistrationRecord>
          bordered
          size="small"
          columns={columns}
      ...
      <Modal
        ...
        destroyOnClose
      >
        <Form<Secondary3YearRegistrationRecord> form={form} layout="vertical">
...
export default Secondary3YearRegistrationRoster
