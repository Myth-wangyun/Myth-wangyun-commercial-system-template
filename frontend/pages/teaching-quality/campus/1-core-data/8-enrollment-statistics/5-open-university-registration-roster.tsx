...
interface OpenUniversityRegistrationRecord {
  key?: string
  serialNumber: string
  studentName: string
  gender: string
...
const OpenUniversityRegistrationRoster: React.FC = () => {
  const { message, modal } = App.useApp()
  const { currentCampus } = useCampusStore()
  const [year, setYear] = useState<number>(new Date().getFullYear())
  const [month, setMonth] = useState<number>(new Date().getMonth() + 1)

  const [dataSource, setDataSource] = useState<OpenUniversityRegistrationRecord[]>([])
  const [isModalVisible, setIsModalVisible] = useState(false)
  const [editingRecord, setEditingRecord] = useState<OpenUniversityRegistrationRecord | null>(null)
  const [form] = Form.useForm<OpenUniversityRegistrationRecord>()
  ...
  const { fetchFromClassFile, loading: classFileLoading } = useClassFileRoster({
    category: 'open-university-registered',
  ...
  const handleEdit = (record: OpenUniversityRegistrationRecord) => {
  ...
  const handleSaveModal = async () => {
    try {
      ...
      const newRecord: OpenUniversityRegistrationRecord = {
        ...editingRecord,
        ...values,
  ...
  const fetchFromServer = async () => {
    ...
    try {
      const res = await fetch(
        buildApiUrl(`/teaching-quality/open-university-registration-roster?campus=${encodeURIComponent(
          currentCampus!,
      ...
      const mapped: OpenUniversityRegistrationRecord[] = rows.map((r: any, idx: number) => ({
        key: String(r.serialNumber ?? idx + 1),
        serialNumber: String(r.serialNumber ?? idx + 1),
        studentName: String(r.studentName || ''),
        gender: String(r.gender || ''),
  ...
  const saveToServer = async () => {
    ...
    try {
      ...
      const res = await fetch(buildApiUrl('/teaching-quality/open-university-registration-roster'), {
        method: 'POST',
  ...
  const columns: ColumnsType<OpenUniversityRegistrationRecord> = [
    {
      title: '序号',
  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      >
        <Table<OpenUniversityRegistrationRecord>
          bordered
          size="small"
          columns={columns}
      ...
      <Modal
        ...
        destroyOnClose
      >
        <Form<OpenUniversityRegistrationRecord> form={form} layout="vertical">
...
export default OpenUniversityRegistrationRoster