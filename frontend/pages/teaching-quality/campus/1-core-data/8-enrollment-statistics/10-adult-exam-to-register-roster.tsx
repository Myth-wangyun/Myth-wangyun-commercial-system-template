...
interface AdultExamToRegisterRecord {
  key: string
...
const AdultExamToRegisterRoster: React.FC = () => {
  const { message, modal } = App.useApp()
  const { currentCampus } = useCampusStore()
  const [dataSource, setDataSource] = useState<AdultExamToRegisterRecord[]>([])
  const [isModalVisible, setIsModalVisible] = useState(false)
  const [editingRecord, setEditingRecord] = useState<AdultExamToRegisterRecord | null>(null)
  const [form] = Form.useForm<AdultExamToRegisterRecord>()
  ...
  const { fetchFromClassFile, loading: classFileLoading } = useClassFileRoster({
    category: 'adult-exam-to-register',
  ...
  const handleEdit = (record: AdultExamToRegisterRecord) => {
  ...
  const handleSaveModal = async () => {
    try {
      ...
      const newRecord: AdultExamToRegisterRecord = {
        ...editingRecord,
        ...formatted,
        key: editingRecord?.key || Date.now().toString(),
  ...
  const fetchFromServer = async () => {
    ...
    try {
      const res = await fetch(
        buildApiUrl(`/teaching-quality/adult-exam-to-register-roster?campus=${encodeURIComponent(currentCampus!)}`),
      )
      if (!res.ok) throw new Error(await res.text())
      const data = await res.json()
      const rows = (data?.行列表 || []) as any[]
      const mapped: AdultExamToRegisterRecord[] = rows.map((r: any, i: number) => ({
  ...
  const saveToServer = async () => {
    ...
    try {
      ...
      const res = await fetch(buildApiUrl('/teaching-quality/adult-exam-to-register-roster'), {
        method: 'POST',
  ...
  const columns: ColumnsType<AdultExamToRegisterRecord> = [
  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      >
        <Table<AdultExamToRegisterRecord>
      ...
      <Modal
        ...
        width={900}
      >
        <Form<AdultExamToRegisterRecord> form={form} layout="vertical">
...
export default AdultExamToRegisterRoster
