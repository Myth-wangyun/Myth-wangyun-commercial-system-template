...
interface Secondary1YearRegistrationRecord {
  key?: string
  schoolName: string
...
const Secondary1YearRegistrationRoster: React.FC = () => {
  const { message, modal } = App.useApp()
  const { currentCampus } = useCampusStore()
  const [year, setYear] = useState<number>(new Date().getFullYear())
  const [month, setMonth] = useState<number>(new Date().getMonth() + 1)

  const [dataSource, setDataSource] = useState<Secondary1YearRegistrationRecord[]>([])
  const [isModalVisible, setIsModalVisible] = useState(false)
  const [editingRecord, setEditingRecord] = useState<Secondary1YearRegistrationRecord | null>(null)
  const [form] = Form.useForm<Secondary1YearRegistrationRecord>()
  ...
  const { fetchFromClassFile, loading: classFileLoading } = useClassFileRoster({
    category: 'secondary-1year-registered',
  ...
  const handleEdit = (record: Secondary1YearRegistrationRecord) => {
  ...
  const handleSaveModal = async () => {
    try {
      ...
      const formatted = {
        ...values,
        registrationTime: values.registrationTime ? (values.registrationTime as any).format('YYYY.MM') : '',
        graduationTime: values.graduationTime ? (values.graduationTime as any).format('YYYY.MM') : '',
        registrationYear: values.registrationYear ? (values.registrationYear as any).format('YYYYMM') : '',
      }
      const newRecord: Secondary1YearRegistrationRecord = {
        ...editingRecord,
        ...formatted,
  ...
  const fetchFromServer = async () => {
    ...
    try {
      const res = await fetch(
        buildApiUrl(`/teaching-quality/secondary-1year-registration-roster?campus=${encodeURIComponent(
          currentCampus!,
      ...
      const mapped: Secondary1YearRegistrationRecord[] = rows.map((r: any) => ({
  ...
  const saveToServer = async () => {
    ...
    try {
      ...
      const res = await fetch(buildApiUrl('/teaching-quality/secondary-1year-registration-roster'), {
        method: 'POST',
  ...
  const columns: ColumnsType<Secondary1YearRegistrationRecord> = [
    {
      title: '学校名称',
  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      >
        <Table<Secondary1YearRegistrationRecord>
          bordered
          size="small"
          columns={columns}
      ...
      <Modal
        ...
        width={900}
      >
        <Form<Secondary1YearRegistrationRecord> form={form} layout="vertical">
...
export default Secondary1YearRegistrationRoster
