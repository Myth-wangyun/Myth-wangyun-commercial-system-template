...
interface OtherSecondaryEducationToRegisterRecord {
  key: string
...
const OtherSecondaryEducationToRegisterRoster: React.FC = () => {
  const { message, modal } = App.useApp()
  const { currentCampus } = useCampusStore()
  const [dataSource, setDataSource] = useState<OtherSecondaryEducationToRegisterRecord[]>([])
  const [isModalVisible, setIsModalVisible] = useState(false)
  const [editingRecord, setEditingRecord] = useState<OtherSecondaryEducationToRegisterRecord | null>(
    null,
  )
  const [form] = Form.useForm<OtherSecondaryEducationToRegisterRecord>()

  const canIO = useMemo(() => Boolean(currentCampus), [currentCampus])

  // 使用自动填充 Hook
  const { getAutoCompleteOptions, handleAutoCompleteSelect, isLoading: isDataLoading, classFileRecords } = useStudentAutoFill(form, {
    genderField: 'gender',
  ...
  const { fetchFromClassFile, loading: classFileLoading } = useClassFileRoster({
    category: 'other-secondary-to-register',
  ...
  const autoCompleteOptions = useMemo(() => {
    const options = getAutoCompleteOptions(studentNameSearchText)
    console.log('[AutoComplete] useMemo 计算选项, 搜索文本:', studentNameSearchText, '选项数量:', options.length, '数据量:', classFileRecords.length)
    return options
  }, [studentNameSearchText, getAutoCompleteOptions, classFileRecords])
  ...
  const handleEdit = (record: OtherSecondaryEducationToRegisterRecord) => {
  ...
  const handleSaveModal = async () => {
    try {
      ...
      const newRecord: OtherSecondaryEducationToRegisterRecord = {
        ...editingRecord,
        ...formatted,
  ...
  const fetchFromServer = async () => {
    ...
    try {
      const res = await fetch(
        buildApiUrl(`/teaching-quality/other-secondary-to-register-roster?campus=${encodeURIComponent(currentCampus!)}`),
      )
      if (!res.ok) throw new Error(await res.text())
      const data = await res.json()
      const rows = (data?.行列表 || []) as any[]
      const mapped: OtherSecondaryEducationToRegisterRecord[] = rows.map((r: any, i: number) => ({
        key: `${i}-${r.idCardNumber || r.studentName || ''}`,
        pendingRegistrationTime: r.pendingRegistrationTime || '',
        studentName: r.studentName || '',
        gender: r.gender || '',
  ...
  const saveToServer = async () => {
    ...
    try {
      ...
      const res = await fetch(buildApiUrl('/teaching-quality/other-secondary-to-register-roster'), {
        method: 'POST',
  ...
  const columns: ColumnsType<OtherSecondaryEducationToRegisterRecord> = [
    {
      title: '需注册时间',
  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      >
        <Table<OtherSecondaryEducationToRegisterRecord>
          bordered
          size="small"
          columns={columns}
      ...
      <Modal
        ...
        width={900}
      >
        <Form<OtherSecondaryEducationToRegisterRecord> form={form} layout="vertical">
          ...
          <Space size="large" style={{ display: 'flex' }}>
            ...
            >
              <AutoComplete
                placeholder={isDataLoading ? "正在加载学生数据..." : "输入姓名搜索并选择学生"}
                options={autoCompleteOptions}
                onSearch={(value) => {
                  console.log('[AutoComplete] onSearch 触发, 值:', value, '当前数据量:', classFileRecords.length)
                  setStudentNameSearchText(value)
                }}
                onSelect={(value, option) => {
                  console.log('[AutoComplete] onSelect 触发:', value, option)
                  handleAutoCompleteSelect(value, option)
                  setStudentNameSearchText('') // 选择后清空搜索文本
                }}
                onFocus={() => {
                  console.log('[AutoComplete] onFocus, 当前搜索文本:', studentNameSearchText, '选项数量:', autoCompleteOptions.length, '数据量:', classFileRecords.length)
                }}
                filterOption={false}
                style={{ width: '100%' }}
                dropdownStyle={{ minWidth: 500, maxWidth: 800 }}
                allowClear
                notFoundContent={studentNameSearchText.trim().length > 0 && !isDataLoading && autoCompleteOptions.length === 0 ? '未找到匹配的学生' : null}
...
export default OtherSecondaryEducationToRegisterRoster
