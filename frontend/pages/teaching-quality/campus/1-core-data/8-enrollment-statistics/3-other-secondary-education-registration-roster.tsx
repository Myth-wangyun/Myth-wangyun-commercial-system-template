...
interface OtherSecondaryEducationRegistrationRecord {
  key?: string
  schoolName: string
...
const OtherSecondaryEducationRegistrationRoster: React.FC = () => {
  const { message, modal } = App.useApp()
  const { currentCampus } = useCampusStore()
  const [year, setYear] = useState<number>(new Date().getFullYear())
  const [month, setMonth] = useState<number>(new Date().getMonth() + 1)

  const [dataSource, setDataSource] = useState<OtherSecondaryEducationRegistrationRecord[]>([])
  const [isModalVisible, setIsModalVisible] = useState(false)
  const [editingRecord, setEditingRecord] = useState<OtherSecondaryEducationRegistrationRecord | null>(null)
  const [form] = Form.useForm<OtherSecondaryEducationRegistrationRecord>()
  ...
  const { fetchFromClassFile, loading: classFileLoading } = useClassFileRoster({
    category: 'other-secondary-registered',
  ...
  const handleEdit = (record: OtherSecondaryEducationRegistrationRecord) => {
    ...
    const mapForForm: any = {
      ...
      registrationYear: record.registrationYear ? dayjs(record.registrationYear, 'YYYYMM') : undefined,
    }
    form.setFieldsValue(mapForForm)
    setIsModalVisible(true)
  }

  const handleDelete = (studentNumber: string) => {
    modal.confirm({
      title: '确认删除',
      content: `确定要删除学号为 ${studentNumber} 的记录吗？`,
      onOk: () => {
        setDataSource((prev) => prev.filter((item) => item.studentNumber !== studentNumber))
  ...
  const handleSaveModal = async () => {
    try {
      ...
      const formatted = {
        ...values,
        registrationTime: values.registrationTime && dayjs.isDayjs(values.registrationTime) ? values.registrationTime.format('YYYY.MM') : '',
        graduationTime: values.graduationTime && dayjs.isDayjs(values.graduationTime) ? values.graduationTime.format('YYYY.MM') : '',
        registrationYear: values.registrationYear && dayjs.isDayjs(values.registrationYear) ? values.registrationYear.format('YYYYMM') : '',
      }
      const newRecord: OtherSecondaryEducationRegistrationRecord = {
        ...editingRecord,
        ...formatted,
        key: values.studentNumber,
        campus: values.campus || currentCampus || '',
      }

      setDataSource((prev) => {
        const exists = prev.some((x) => x.studentNumber === newRecord.studentNumber)
        if (editingRecord || exists) {
          return prev.map((item) => (item.studentNumber === (editingRecord?.studentNumber || newRecord.studentNumber) ? newRecord : item))
        }
  ...
  const fetchFromServer = async () => {
    ...
    try {
      const res = await fetch(
        buildApiUrl(`/teaching-quality/other-secondary-registration-roster?campus=${encodeURIComponent(
          currentCampus!,
      ...
      const mapped: OtherSecondaryEducationRegistrationRecord[] = rows.map((r: any) => ({
  ...
  const saveToServer = async () => {
    ...
    try {
      ...
      const res = await fetch(buildApiUrl('/teaching-quality/other-secondary-registration-roster'), {
        method: 'POST',
  ...
  const columns: ColumnsType<OtherSecondaryEducationRegistrationRecord> = [
    {
      title: '学校名称',
      ...
      render: (_, record) => (
        <Space size="small">
          ...
          <Button
            ...
            danger
            icon={<DeleteOutlined />}
            onClick={() => handleDelete(record.studentNumber)}
          >
            删除
          </Button>
        </Space>
  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      >
        <Table<OtherSecondaryEducationRegistrationRecord>
          bordered
          size="small"
          columns={columns}
      ...
      <Modal
        ...
        destroyOnClose
      >
        <Form<OtherSecondaryEducationRegistrationRecord> form={form} layout="vertical">
          ...
          <Space size="large" style={{ display: 'flex' }}>
            ...
            <Form.Item name="registrationYear" label="注册年份">
              <DatePicker picker="month" format="YYYYMM" style={{ width: 140 }} />
            </Form.Item>
          </Space>
...
export default OtherSecondaryEducationRegistrationRoster
