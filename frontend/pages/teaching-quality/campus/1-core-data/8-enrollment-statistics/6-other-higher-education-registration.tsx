...
interface OtherHigherEducationRegistrationRecord {
  key?: string
  serialNumber: string
  studentName: string
  gender: string
...
const OtherHigherEducationRegistrationRoster: React.FC = () => {
  const { message, modal } = App.useApp()
  const { currentCampus } = useCampusStore()
  const [year, setYear] = useState<number>(new Date().getFullYear())
  const [month, setMonth] = useState<number>(new Date().getMonth() + 1)

  const [dataSource, setDataSource] = useState<OtherHigherEducationRegistrationRecord[]>([])
  const [isModalVisible, setIsModalVisible] = useState(false)
  const [editingRecord, setEditingRecord] = useState<OtherHigherEducationRegistrationRecord | null>(
    null,
  )
  const [form] = Form.useForm<OtherHigherEducationRegistrationRecord>()
  ...
  const { fetchFromClassFile, loading: classFileLoading } = useClassFileRoster({
    category: 'other-higher-registered',
    campus: currentCampus,
    mapFunction: (rawData, campus) => rawData.map((r: any, idx: number) => ({
  ...
  const handleEdit = (record: OtherHigherEducationRegistrationRecord) => {
  ...
  const handleSaveModal = async () => {
    try {
      ...
      const newRecord: OtherHigherEducationRegistrationRecord = {
        ...editingRecord,
        ...values,
  ...
  const fetchFromServer = async () => {
    ...
    try {
      const res = await fetch(
        buildApiUrl(`/teaching-quality/other-higher-education-registration-roster?campus=${encodeURIComponent(
          currentCampus!,
      ...
      const mapped: OtherHigherEducationRegistrationRecord[] = rows.map((r: any, idx: number) => ({
        key: String(r.serialNumber ?? idx + 1),
        serialNumber: String(r.serialNumber ?? idx + 1),
        studentName: String(r.studentName || ''),
        gender: String(r.gender || ''),
  ...
  const saveToServer = async () => {
    ...
    try {
      ...
      const res = await fetch(buildApiUrl('/teaching-quality/other-higher-education-registration-roster'), {
        method: 'POST',
  ...
  const columns: ColumnsType<OtherHigherEducationRegistrationRecord> = [
    {
      title: '序号',
  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      >
        <Table<OtherHigherEducationRegistrationRecord>
          bordered
          size="small"
          columns={columns}
      ...
      <Modal
        ...
        destroyOnClose
      >
        <Form<OtherHigherEducationRegistrationRecord> form={form} layout="vertical">
...
export default OtherHigherEducationRegistrationRoster