// 神殿 -> 教化司 · QMJY-JZ-007  XX神殿教化司学员异动表 长期请假明细表
...
const hasRowData = (row: LongLeaveDetailRow): boolean => {
  return !!(
    row.studentName ||
    row.gender ||
    row.originalClass ||
...
const createInitialRows = (count = 25): LongLeaveDetailRow[] =>
...
const CampusLongLeaveDetailTable: React.FC = () => {
  const { message } = App.useApp()
  const { currentCampus } = useCampusStore()
  const [year, setYear] = useState<number>(new Date().getFullYear())
  const [month, setMonth] = useState<number>(new Date().getMonth() + 1)
  const [rows, setRows] = useState<LongLeaveDetailRow[]>(createInitialRows())
  ...
  const handleTextChange = (key: string, field: keyof LongLeaveDetailRow, value: string) => {
    setRows((prev) =>
      prev.map((row) =>
        row.key === key
          ? {
  ...
  const handleNumberChange = (
    key: string,
    field: keyof Pick<LongLeaveDetailRow, 'serialNumber' | 'receivableTuition' | 'paidTuition'>,
    value: number | null,
  ) => {
  ...
  // 从班档案表导入长期请假学生
  const importFromClassFile = async (): Promise<LongLeaveDetailRow[]> => {
    if (!currentCampus) {
    ...
    try {
      const res = await fetch(
        buildApiUrl(`/teaching-quality/class-file/students-by-status?campus=${encodeURIComponent(currentCampus)}&status=长期请假`)
      )
      ...
      const importedRows: LongLeaveDetailRow[] = students.map((student, idx) => ({
  ...
  const addOneRow = () => {
    ...
    const newRow: LongLeaveDetailRow = {
  ...
  const mapRow = (item: any, idx: number, m: number): LongLeaveDetailRow => ({
    _month: m,
    key: `${m}-${idx + 1}`,
    ...
    leaveStartDate: String(item.leaveStartDate || ''),
    leaveEndDate: String(item.leaveEndDate || ''),
    ...
    leaveDescription: String(item.leaveDescription || ''),
  ...
  const fetchFromServer = async (): Promise<LongLeaveDetailRow[]> => {
    if (!canIO) {
    ...
    try {
      ...
      const urls = months.map((m) => buildApiUrl(`/teaching-quality/campus-long-leave-detail?campus=${encodeURIComponent(currentCampus!)}&year=${year}&month=${m}`))
      const resList = await Promise.all(urls.map((u) => fetch(u).catch(() => null)))
      const allRows: LongLeaveDetailRow[] = []
    ...
    try {
      const payload = {
        ...
        行列表: rows.map((r, idx) => ({
          ...
          leaveDescription: r.leaveDescription,
      ...
      const res = await fetch(buildApiUrl('/teaching-quality/campus-long-leave-detail'), {
        method: 'POST',
  ...
  const mergeData = (serverData: LongLeaveDetailRow[], classFileData: LongLeaveDetailRow[]): LongLeaveDetailRow[] => {
    const idCardMap = new Map<string, LongLeaveDetailRow>()
  ...
  const columns: ColumnsType<LongLeaveDetailRow> = [
    {
      title: '月份',
      key: 'month-label',
      width: 80,
      ...
      fixed: 'left',
      render: (_: any, record) => {
        if ((record as any)._month !== undefined) return (record as any)._month
      ...
      children: [
        ...
        },
        {
          ...
          render: (text: string, record) => (
            <DatePicker
              ...
              onChange={(_, dateString) => handleTextChange(record.key, 'leaveStartDate', dateString as string)}
              style={{ width: '100%' }}
              format={DATE_FORMAT}
            />
          ),
        },
        {
          ...
          render: (text: string, record) => (
            <DatePicker
              value={text ? dayjs(text, DATE_FORMAT) : null}
              onChange={(_, dateString) => handleTextChange(record.key, 'leaveEndDate', dateString as string)}
              style={{ width: '100%' }}
              format={DATE_FORMAT}
            />
          ),
        ...
        },
        {
          ...
          render: (text: string, record) => (
            <Input
              value={text}
              onChange={(e) =>
                handleTextChange(record.key, 'returnClassHeadTeacher', e.target.value)
              }
            />
          ),
        },
        ...
        },
        {
          ...
          render: (text: string, record) => (
            <Input
              value={text}
              onChange={(e) =>
                handleTextChange(record.key, 'completedCoursesNames', e.target.value)
              }
              style={{ borderColor: !text || !text.trim() ? '#ff4d4f' : undefined }}
            />
          ),
        ...
        },
        {
          ...
          render: (text: string, record) => (
            <Input.TextArea
              ...
              value={text}
              onChange={(e) => handleTextChange(record.key, 'leaveDescription', e.target.value)}
              style={{ borderColor: !text || !text.trim() ? '#ff4d4f' : undefined }}
            />
          ),
        },
        {
          ...
          render: (text: string, record) => (
            <Input.TextArea
              ...
              value={text}
              onChange={(e) =>
                handleTextChange(record.key, 'studentParentThoughts', e.target.value)
              }
              style={{ borderColor: !text || !text.trim() ? '#ff4d4f' : undefined }}
            />
          ),
  ...
  return (
    <div style={{ padding: 24 }}>
      <Card
        title={`07-4 ${currentCampus || ''} 教质长期请假明细表`}
        extra={
          <Space>
            <span>年份</span>
            <InputNumber
...
export default CampusLongLeaveDetailTable
