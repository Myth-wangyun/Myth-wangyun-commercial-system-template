// 神殿 -> 教化司 · QMJY-JZ-007  XX神殿教化司学员异动表 长期不上课明细表
...
interface LongAbsenceDetailRow {
  _month?: number
...
const hasRowData = (row: LongAbsenceDetailRow): boolean => {
  return !!(
    row.studentName ||
    row.gender ||
    row.originalClass ||
    row.originalHeadTeacher ||
    row.absenceStartDate ||
    row.contactPhone ||
...
const createInitialRows = (count = 25): LongAbsenceDetailRow[] =>
...
const CampusLongAbsenceDetailTable: React.FC = () => {
  const { message } = App.useApp()
  const { currentCampus } = useCampusStore()
  const [year, setYear] = useState<number>(new Date().getFullYear())
  const [month, setMonth] = useState<number>(new Date().getMonth() + 1)
  const [rows, setRows] = useState<LongAbsenceDetailRow[]>(createInitialRows())
  ...
  const handleTextChange = (key: string, field: keyof LongAbsenceDetailRow, value: string) => {
    setRows((prev) =>
      prev.map((row) =>
        row.key === key
          ? {
  ...
  const handleNumberChange = (
    key: string,
    field: keyof Pick<LongAbsenceDetailRow, 'serialNumber' | 'receivableTuition' | 'paidTuition'>,
    value: number | null,
  ) => {
  ...
  // 从班档案表导入长期不上课学生
  const importFromClassFile = async (): Promise<LongAbsenceDetailRow[]> => {
    if (!currentCampus) {
    ...
    try {
      const res = await fetch(
        buildApiUrl(`/teaching-quality/class-file/students-by-status?campus=${encodeURIComponent(currentCampus)}&status=长期不上课`)
      )
      ...
      const importedRows: LongAbsenceDetailRow[] = students.map((student, idx) => ({
  ...
  const addOneRow = () => {
    ...
    const newRow: LongAbsenceDetailRow = {
  ...
  const mapRow = (item: any, idx: number, m: number): LongAbsenceDetailRow => ({
    _month: m,
    key: `${m}-${idx + 1}`,
    ...
    absenceDescription: String(item.absenceDescription || ''),
  ...
  const fetchFromServer = async (): Promise<LongAbsenceDetailRow[]> => {
    if (!canIO) {
    ...
    try {
      ...
      const urls = months.map((m) =>
        buildApiUrl(`/teaching-quality/campus-long-absence-detail?campus=${encodeURIComponent(currentCampus!)}&year=${year}&month=${m}`),
      )
      const resList = await Promise.all(urls.map((u) => fetch(u).catch(() => null)))
      const allRows: LongAbsenceDetailRow[] = []
    ...
    try {
      const payload = {
        ...
        行列表: rows.map((r, idx) => ({
          ...
          absenceStartDate: r.absenceStartDate,
          ...
          absenceDescription: r.absenceDescription,
      ...
      const res = await fetch(buildApiUrl('/teaching-quality/campus-long-absence-detail'), {
        method: 'POST',
  ...
  const mergeData = (serverData: LongAbsenceDetailRow[], classFileData: LongAbsenceDetailRow[]): LongAbsenceDetailRow[] => {
    const idCardMap = new Map<string, LongAbsenceDetailRow>()
  ...
  const columns: ColumnsType<LongAbsenceDetailRow> = [
    {
      title: '月份',
      key: 'month-label',
      width: 80,
    ...
    },
    {
      ...
      children: [
        ...
        {
          title: '不来上课开始日期',
          dataIndex: 'absenceStartDate',
          key: 'absenceStartDate',
          width: 150,
          align: 'center',
          render: (text: string, record) => (
            <DatePicker
              value={text ? dayjs(text, DATE_FORMAT) : null}
              onChange={(_, dateString) => handleTextChange(record.key, 'absenceStartDate', dateString as string)}
              style={{ width: '100%' }}
              format={DATE_FORMAT}
            />
          ),
    ...
    },
    {
      ...
      children: [
        ...
        {
          title: '学生长期不上课情况说明（详细）',
          dataIndex: 'absenceDescription',
          key: 'absenceDescription',
          width: 230,
          align: 'left',
          render: (text: string, record) => (
            <Input.TextArea
              ...
              value={text}
              onChange={(e) => handleTextChange(record.key, 'absenceDescription', e.target.value)}
              style={{ borderColor: !text || !text.trim() ? '#ff4d4f' : undefined }}
            />
          ),
        },
  ...
  return (
    <div style={{ padding: 24 }}>
      <Card title={`07-5 ${currentCampus || ''} 教质长期不来上课明细表`} style={{ backgroundColor: '#fff' }}
        extra={
          <Space>
            <span>年份</span>
            <InputNumber
...
export default CampusLongAbsenceDetailTable
