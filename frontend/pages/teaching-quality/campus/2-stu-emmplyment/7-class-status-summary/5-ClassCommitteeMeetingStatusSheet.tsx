// 班委会情况：ClassCommitteeMeetingStatusSheet.tsx · 班委会情况表（按班级+年月）
// 功能：选择班级/年月、读取/刷新、编辑与保存。
...
const createRow = (
  serial: number,
  overrides: Partial<CommitteeMeetingRow> = {},
): CommitteeMeetingRow => ({
  key: String(serial),
  serialNumber: serial,
  time: '',
...
const initialData: CommitteeMeetingRow[] = Array.from({ length: DEFAULT_ROWS_COUNT }, (_, i) => createRow(i + 1))

const fromApiRow = (r: ApiRow): CommitteeMeetingRow => ({
  key: String(r.序号),
  serialNumber: r.序号,
  time: (r.时间 || '').trim(),
  location: (r.地点 || '').trim(),
...
const toApiRow = (r: CommitteeMeetingRow): ApiRow => ({
  序号: r.serialNumber,
  时间: r.time || undefined,
...
const ClassCommitteeMeetingStatusSheet: React.FC = () => {
  ...
  const [dataSource, setDataSource] = useState<CommitteeMeetingRow[]>(initialData)
  ...
  // 解析粘贴的数据
  const parseImportData = (text: string): CommitteeMeetingRow[] => {
    const lines = text.trim().split('\n').filter(line => line.trim())
    if (lines.length === 0) return []

    const rows: CommitteeMeetingRow[] = []
    let serialCounter = 1
  ...
  const ensureDefaultRows = (rows: CommitteeMeetingRow[]): CommitteeMeetingRow[] => {
  ...
  const loadData = async (campus: string, klass: string, y: number, m: number) => {
    ...
    try {
      const res = await fetch(
        buildApiUrl(`/teaching-quality/class-committee-meeting-status?campus=${encodeURIComponent(campus)}&class=${encodeURIComponent(klass)}&year=${y}&month=${m}`),
      )
      ...
      console.error(e)
      message.warning('未能读取班委会情况，使用默认空白行')
  ...
  const handleChange = (key: string, field: keyof CommitteeMeetingRow, value: string) => {
    setDataSource((prev) => prev.map((row) => (row.key === key ? { ...row, [field]: value } : row)))
  }

  const columns: ColumnsType<CommitteeMeetingRow> = [
    {
      title: '时间',
      dataIndex: 'time',
      key: 'time',
  ...
  const anyContentFilled = (r: CommitteeMeetingRow) => !!((r.time && r.time.trim()) || (r.location && r.location.trim()) || (r.participants && r.participants.trim()) || (r.topic && r.topic.trim()) || (r.keyPoints && r.keyPoints.trim()))
          ...
          <Space>
            ...
            <Button
              ...
              onClick={async () => {
                ...
                try {
                  const res = await fetch(buildApiUrl('/teaching-quality/class-committee-meeting-status'), {
                    method: 'POST',
...
export default ClassCommitteeMeetingStatusSheet
