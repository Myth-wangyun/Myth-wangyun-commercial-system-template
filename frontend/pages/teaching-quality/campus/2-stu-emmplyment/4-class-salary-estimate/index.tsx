...
const extractBirthDateFromIdCard = (idCard: string): string => {
  ...
  if (trimmed.length === 15) {
    const year = '19' + trimmed.substring(6, 8);
    const month = trimmed.substring(8, 10);
    return `${year}-${month}`;
  }
...
const mergeStudentInfoFromClassFile = (
  ...
  classFileRows.forEach((row) => {
    ...
    // 从身份证号自动提取出生年月
    const idCard = row.idCard || existing.idCard || '';
    const birthDate = idCard ? extractBirthDateFromIdCard(idCard) : '';

    baseMap.set(name, {
      ...existing,
      serialNumber: existing.serialNumber || row.serialNumber || existing.serialNumber,
      // 只在为空时从档案表补齐，避免覆盖手动编辑/后端已有数据
      idCard: existing.idCard || idCard || '',
