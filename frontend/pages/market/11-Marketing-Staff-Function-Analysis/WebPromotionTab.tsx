...
interface WebPromotionTabProps {
  year: string
  month: string
}
...
// 网推功能分析数据
const webPromotionData: FunctionAnalysisRow[] = [
  // 核心业务能力 (1-4行，共4行)
  { id: 1, category: '核心业务能力', categoryRowSpan: 4, functionItem: '招生收入', detailRequirement: '年度整理招生收入任务完成情况', fullScore: 10 },
  { id: 2, category: '核心业务能力', functionItem: '完成咨询量', detailRequirement: '年度完成咨询量的能力', fullScore: 5 },
  { id: 3, category: '核心业务能力', functionItem: '招生成本', detailRequirement: '年度把控净报名成本', fullScore: 10 },
  { id: 4, category: '核心业务能力', functionItem: '投产比', detailRequirement: '年度投产比情况', fullScore: 10 },
  // 一般业务能力 (5-8行，共4行)
  { id: 5, category: '一般业务能力', categoryRowSpan: 4, functionItem: '学习+培训', detailRequirement: '自身学习+培训分享能力', fullScore: 5 },
  { id: 6, category: '一般业务能力', functionItem: '咨询对接会', detailRequirement: '年度咨询对接会情况', fullScore: 5 },
  { id: 7, category: '一般业务能力', functionItem: '退费率', detailRequirement: '年度退费情况', fullScore: 5 },
...
const scoringStandardData: ScoringStandardRow[] = [
  // 核心业务能力 (1-4行，共4行)
  { id: 1, category: '核心业务能力', categoryRowSpan: 4, scoringItem: '招生收入', scoringStandard: '年度收入完成率≥110%为优，年度收入完成率≥100%为良，年度收入完成率≥90%为为合格，年度收入完成率<90%为为差' },
  { id: 2, category: '核心业务能力', scoringItem: '完成咨询量', scoringStandard: '咨询量完成率≥110%为优，咨询量完成率≥100%为良，咨询量完成率≥90%为合格，咨询量完成率<9%为差' },
  { id: 3, category: '核心业务能力', scoringItem: '招生成本', scoringStandard: '招生成本≤5000为优，招生成本≤5500为良，招生成本≤6500为合格，招生成本>6500为差' },
  { id: 4, category: '核心业务能力', scoringItem: '投产比', scoringStandard: 'ROI≥5以上为优，ROI≥4以上为良，ROI≥3合格，ROI<3差' },
  // 一般业务能力 (5-8行，共4行)
  { id: 5, category: '一般业务能力', categoryRowSpan: 4, scoringItem: '学习+培训', scoringStandard: '上级领导进行打分' },
  { id: 6, category: '一般业务能力', scoringItem: '咨询对接会', scoringStandard: '咨询对接执行分≥9为优，咨询对接执行分≥8为良，达咨询对接执行分≥7为合格，咨询对接执行分<7为差' },
  { id: 7, category: '一般业务能力', scoringItem: '退费率', scoringStandard: '退费率10%以下为优，20%以下为良，30%以下为合格，高于30%为差' },
  { id: 8, category: '一般业务能力', scoringItem: '数据分析', scoringStandard: '当月重点人群转化占比' },
  // 价值观 (9-17行，共9行)
...
const defaultEmployees: Employee[] = [
  { key: 'kangQing', name: '康青' },
  { key: 'zhangLichen', name: '张立臣' },
  { key: 'yinWentuan', name: '殷立园' },
  { key: 'yuDongfeng', name: '郝东风' },
  { key: 'xiaYuehua', name: '薛跃华' },
]

const WebPromotionTab: React.FC<WebPromotionTabProps> = ({ year, month }) => {
  ...
  const loadData = useCallback(async () => {
    setLoading(true)
    try {
      const response = await marketStaffFunctionService.webPromotion.getData(year)
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await marketStaffFunctionService.webPromotion.save({
        year,
  ...
  const dataWithTotal = useMemo(() => {
    const totalRow: FunctionAnalysisRow = {
      ...
      functionItem: '',
      detailRequirement: '',
      fullScore: webPromotionData.reduce((sum, row) => sum + row.fullScore, 0),
    }
    // 为每个员工计算总分
    employees.forEach(emp => {
      totalRow[emp.key] = calculateEmployeeTotal(emp.key)
    })
    return [...webPromotionData, totalRow]
  }, [employees, scores])
  ...
  return (
    <Spin spinning={loading}>
      <div>
        ...
        >
          <span style={{ flex: 1, textAlign: 'center' }}>{year}年市场部-网推功能分析表</span>
          <Button
            type="primary"
            icon={<SaveOutlined />}
            onClick={handleSave}
...
export default WebPromotionTab
