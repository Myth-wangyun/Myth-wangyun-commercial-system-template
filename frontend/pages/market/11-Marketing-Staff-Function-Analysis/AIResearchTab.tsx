...
// AI研发+其他功能分析数据
const aiResearchData: FunctionAnalysisRow[] = [
  // 核心业务能力 (1-4行，共4行)
  { id: 1, category: '核心业务能力', categoryRowSpan: 4, functionItem: '任务完成率', detailRequirement: '年度/月度完成率', fullScore: 10 },
  { id: 2, category: '核心业务能力', functionItem: '交付率', detailRequirement: '月度交付完成情况', fullScore: 10 },
  { id: 3, category: '核心业务能力', functionItem: '专业度', detailRequirement: '年度专业度胜任情况', fullScore: 10 },
  { id: 4, category: '核心业务能力', functionItem: '创新', detailRequirement: '年度创新的个数排名', fullScore: 5 },
  // 一般业务能力 (5-8行，共4行)
  { id: 5, category: '一般业务能力', categoryRowSpan: 4, functionItem: '学习+培训', detailRequirement: '自身学习+培训分享能力', fullScore: 5 },
  { id: 6, category: '一般业务能力', functionItem: '日常行为', detailRequirement: '日常行为规范执行符合度', fullScore: 5 },
...
const scoringStandardData: ScoringStandardRow[] = [
  // 核心业务能力 (1-4行，共4行)
  { id: 1, category: '核心业务能力', categoryRowSpan: 4, scoringItem: '任务完成率', scoringStandard: '年度/月度领导制定的任务完成率' },
  { id: 2, category: '核心业务能力', scoringItem: '交付率', scoringStandard: '年度/月度领导制定的任务交付率' },
  { id: 3, category: '核心业务能力', scoringItem: '专业度', scoringStandard: '年度/月度匹配工作的专业度' },
  { id: 4, category: '核心业务能力', scoringItem: '创新', scoringStandard: '年度创新的个数排名' },
  // 一般业务能力 (5-8行，共4行)
  { id: 5, category: '一般业务能力', categoryRowSpan: 4, scoringItem: '学习+培训', scoringStandard: '上级领导进行打分' },
  { id: 6, category: '一般业务能力', scoringItem: '日常行为', scoringStandard: '根据市场部日常行为规范执行情况' },
...
const defaultEmployees: Employee[] = [
  { key: 'wangWei', name: '王伟' },
  { key: 'chenLei', name: '陈雷' },
  { key: 'wangZexi', name: '王泽熙' },
  { key: 'zhangYuhang', name: '张宇行' },
]

const AIResearchTab: React.FC<AIResearchTabProps> = ({ year, month }) => {
  ...
  const loadData = useCallback(async () => {
    setLoading(true)
    try {
      const response = await marketStaffFunctionService.aiResearch.getData(year)
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await marketStaffFunctionService.aiResearch.save({
        year,
  ...
  const dataWithTotal = useMemo(() => {
    const totalRow: FunctionAnalysisRow = {
      ...
      functionItem: '',
      detailRequirement: '',
      fullScore: aiResearchData.reduce((sum, row) => sum + row.fullScore, 0),
    }
    // 为每个员工计算总分
    employees.forEach(emp => {
      totalRow[emp.key] = calculateEmployeeTotal(emp.key)
    })
    return [...aiResearchData, totalRow]
  }, [employees, scores])
  ...
  return (
    <Spin spinning={loading}>
      <div>
        ...
        >
          <span style={{ flex: 1, textAlign: 'center' }}>{year}年市场部员工功能分析表</span>
          <Button
            type="primary"
            icon={<SaveOutlined />}
            onClick={handleSave}
        ...
        <Table
          ...
          bordered
          size="small"
          scroll={{ x: 1100 + employees.length * 100 }}
