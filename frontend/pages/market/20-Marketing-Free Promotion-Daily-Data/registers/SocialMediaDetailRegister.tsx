...
import { App, Table, Button, Form, Input, DatePicker, InputNumber, Modal, Space, Spin, Select } from 'antd'
...
const SocialMediaDetailRegister: React.FC<SocialMediaDetailProps> = ({ campusId, campusName, selectedMonth, onMonthChange }) => {
  ...
  const loadData = async () => {
    ...
    setLoading(true)
    try {
      const response = await axios.get(`${API_BASE}/social-media-detail/list`, {
        params: {
          campus: campusName,
  ...
  const handleSave = async () => {
    ...
    setSaving(true)
    try {
      const response = await axios.post(`${API_BASE}/social-media-detail/save`, {
        campus: campusName,
        month: selectedMonth.format('YYYY-MM'),
        items: dataSource
      })
  ...
  return (
    <div>
      <h3 style={{ marginBottom: 16 }}>{campusName}-{selectedMonth.format('M')}月社交新媒体登记明细表</h3>
      ...
      >
        <Form form={form} layout="vertical">
          ...
          <Form.Item
            label="主题/题目"
            name="主题题目"
            rules={[{ required: true, message: '请输入主题/题目' }]}
          >
            <Input.TextArea rows={2} placeholder="请输入视频主题或题目" />
          </Form.Item>
          
          <Form.Item label="剪辑短视频名称" name="剪辑短视频名称">
...
export default SocialMediaDetailRegister
