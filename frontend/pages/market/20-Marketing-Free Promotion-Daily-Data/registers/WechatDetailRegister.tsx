...
const WechatDetailRegister: React.FC<WechatDetailProps> = ({ campusId, campusName, selectedMonth, onMonthChange }) => {
  ...
  const loadData = async () => {
    ...
    setLoading(true)
    try {
      const response = await axios.get(`${API_BASE}/wechat-detail/list`, {
        params: {
          campus: campusName,
  ...
  const columns: ColumnsType<DetailRecord> = [
    ...
    { title: '主题/题目', dataIndex: '主题题目', width: 200, align: 'center' },
    { title: '剪辑短视频/小程序名称', dataIndex: '剪辑短视频小程序名称', width: 180, align: 'center' },
  ...
  const handleSave = async () => {
    ...
    setSaving(true)
    try {
      const response = await axios.post(`${API_BASE}/wechat-detail/save`, {
        campus: campusName,
        month: selectedMonth.format('YYYY-MM'),
        items: dataSource
      })
  ...
  return (
    <div>
      <h3 style={{ marginBottom: 16 }}>{campusName}-{selectedMonth.format('M')}月微信平台登记明细表</h3>
      ...
      <Modal title={editingRecord ? '编辑记录' : '新增记录'} open={isModalVisible} onOk={handleOk} onCancel={handleCancel} width={600}>
        <Form form={form} layout="vertical">
          ...
          <Form.Item label="主题/题目" name="主题题目" rules={[{ required: true, message: '请输入主题/题目' }]}>
            <Input.TextArea rows={2} placeholder="请输入视频或文章主题" />
          </Form.Item>
          <Form.Item label="剪辑短视频/小程序名称" name="剪辑短视频小程序名称">
            <Input placeholder="请输入名称" />
          </Form.Item>
          <Form.Item label="秒" name="秒">
            <InputNumber min={0} style={{ width: '100%' }} placeholder="视频时长(秒)" />
          </Form.Item>
          <Form.Item label="发布链接" name="发布链接">
...
export default WechatDetailRegister
