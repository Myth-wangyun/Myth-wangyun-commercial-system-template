...
const VideoDetailRegister: React.FC<VideoDetailProps> = ({ campusId, campusName, selectedMonth, onMonthChange }) => {
  ...
  const loadData = async () => {
    ...
    setLoading(true)
    try {
      const response = await axios.get(`${API_BASE}/video-detail/list`, {
        params: {
          campus: campusName,
  ...
  const columns: ColumnsType<DetailRecord> = [
    ...
    { title: '重点人群', dataIndex: '重点人群', width: 100, align: 'center' },
    { title: '视频主题/标题', dataIndex: '视频主题标题', width: 200, align: 'center' },
    { title: '剪辑短/长视频名称', dataIndex: '剪辑短长视频名称', width: 150, align: 'center' },
    { title: '秒', dataIndex: '秒', width: 80, align: 'center' },
    {
      title: '备注链接',
      dataIndex: '备注链接',
      width: 250,
    ...
    { title: '有效数', dataIndex: '有效数', width: 80, align: 'center' },
    { title: '备注', dataIndex: '备注', width: 150, align: 'center' },
    {
      title: '操作',
      width: 150,
      fixed: 'right',
  ...
  const handleEdit = (record: DetailRecord) => {
    ...
    form.setFieldsValue({
      ...
      发布平台: record.发布平台,
      重点人群: record.重点人群,
      视频主题标题: record.视频主题标题,
      剪辑短长视频名称: record.剪辑短长视频名称,
      秒: record.秒,
      备注链接: record.备注链接,
      有效数: record.有效数,
      备注: record.备注
      ...
      if (editingRecord) {
        setDataSource(dataSource.map(item => 
          item.key === editingRecord.key ? {
            ...
            发布平台: values.发布平台,
            重点人群: values.重点人群,
            视频主题标题: values.视频主题标题,
            剪辑短长视频名称: values.剪辑短长视频名称,
            秒: values.秒,
            备注链接: values.备注链接,
            有效数: values.有效数,
            备注: values.备注
  ...
  const handleSave = async () => {
    ...
    setSaving(true)
    try {
      const response = await axios.post(`${API_BASE}/video-detail/save`, {
        campus: campusName,
        month: selectedMonth.format('YYYY-MM'),
        items: dataSource
      })
  ...
  return (
    <div>
      <h3 style={{ marginBottom: 16 }}>{campusName}-{selectedMonth.format('M')}月视频推广登记明细表</h3>
      ...
      <Table columns={columns} dataSource={dataSource} pagination={false} scroll={{ x: 1500 }} bordered size="small" loading={loading} />
      ...
      <Modal title={editingRecord ? '编辑记录' : '新增记录'} open={isModalVisible} onOk={handleOk} onCancel={handleCancel} width={600}>
        <Form form={form} layout="vertical">
          ...
          <Form.Item label="发布平台" name="发布平台" rules={[{ required: true, message: '请选择发布平台' }]}>
            <Select placeholder="请选择发布平台">
              <Select.Option value="爱奇艺">爱奇艺</Select.Option>
              <Select.Option value="优酷">优酷</Select.Option>
            </Select>
          </Form.Item>
          ...
          <Form.Item label="视频主题/标题" name="视频主题标题" rules={[{ required: true, message: '请输入视频主题/标题' }]}>
            <Input.TextArea rows={2} placeholder="请输入视频主题或标题" />
          </Form.Item>
          <Form.Item label="剪辑短/长视频名称" name="剪辑短长视频名称">
            <Input placeholder="请输入视频名称" />
          </Form.Item>
          <Form.Item label="秒" name="秒">
            <InputNumber min={0} style={{ width: '100%' }} placeholder="视频时长(秒)" />
          </Form.Item>
          <Form.Item label="备注链接" name="备注链接">
...
export default VideoDetailRegister
