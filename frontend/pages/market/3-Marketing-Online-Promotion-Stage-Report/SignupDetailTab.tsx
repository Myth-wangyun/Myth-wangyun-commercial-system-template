...
import { getSignupDetailRecords } from './api'
import type { ConsultationRecord } from './api'

interface SignupDetailRecord {
  key: string
  signupDate: string
  ...
  reputationSignup: string
  channelSignup: string
  name: string
  age: string
  gender: string
  phone: string
...
interface SignupDetailTabProps {
  campusId: string
  startDate: Dayjs
  endDate: Dayjs
}

const SignupDetailTab: React.FC<SignupDetailTabProps> = ({ campusId, startDate, endDate }) => {
  const { message } = App.useApp()
  const [loading, setLoading] = useState(false)
  const [dataSource, setDataSource] = useState<SignupDetailRecord[]>([])

  // 将API返回的数据转换为表格显示格式
  const transformRecords = (records: ConsultationRecord[]): SignupDetailRecord[] => {
    // 按日期排序
  ...
  const fetchData = async () => {
    try {
      ...
      // 从咨询系统获取报名明细数据
      const records = await getSignupDetailRecords(campusId, startDateStr, endDateStr)
      const transformedData = transformRecords(records)
      
      setDataSource(transformedData)
      
      if (transformedData.length > 0) {
        message.success(`成功加载 ${transformedData.length} 条报名记录`)
      } else {
        message.info('当前条件下没有报名记录')
      }
    } catch (error) {
  ...
  // 表格列定义
  const columns: ColumnsType<SignupDetailRecord> = [
    {
      title: '报名日期',
    ...
    {
      title: '口碑报名',
      dataIndex: 'reputationSignup',
      key: 'reputationSignup',
      align: 'center',
    },
    {
      title: '渠道报名',
      dataIndex: 'channelSignup',
      key: 'channelSignup',
      align: 'center',
    },
    {
...
export default SignupDetailTab
