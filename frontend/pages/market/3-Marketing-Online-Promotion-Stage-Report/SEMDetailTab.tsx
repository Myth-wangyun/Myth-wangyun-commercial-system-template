...
import { getSEMDetailRecords } from './api'
import type { ConsultationRecord } from './api'

interface SEMDetailRecord {
  key: string
  date: string
...
interface SEMDetailTabProps {
  campusId: string
  startDate: Dayjs
  endDate: Dayjs
}

const SEMDetailTab: React.FC<SEMDetailTabProps> = ({ campusId, startDate, endDate }) => {
  const { message } = App.useApp()
  const [loading, setLoading] = useState(false)
  const [dataSource, setDataSource] = useState<SEMDetailRecord[]>([])

  // 将API返回的数据转换为表格显示格式
  const transformRecords = (records: ConsultationRecord[]): SEMDetailRecord[] => {
    // 按日期排序
  ...
  const fetchData = async () => {
    try {
      ...
      // 从咨询系统获取SEM明细数据
      const records = await getSEMDetailRecords(campusId, startDateStr, endDateStr)
      const transformedData = transformRecords(records)
      
      setDataSource(transformedData)
      
      if (transformedData.length > 0) {
        message.success(`成功加载 ${transformedData.length} 条SEM咨询记录`)
      } else {
        message.info('当前条件下没有SEM咨询记录')
      }
    } catch (error) {
  ...
  // 表格列定义
  const columns: ColumnsType<SEMDetailRecord> = [
    {
      title: '日期',
      dataIndex: 'date',
      key: 'date',
