...
import { getVisitDetailRecords } from './api'
import type { ConsultationRecord } from './api'

interface VisitDetailRecord {
  key: string
  date: string
...
const VisitDetailTab: React.FC<VisitDetailTabProps> = ({ campusId, startDate, endDate }) => {
  const { message } = App.useApp()
  const [loading, setLoading] = useState(false)
  const [dataSource, setDataSource] = useState<VisitDetailRecord[]>([])

  // 将API返回的数据转换为表格显示格式
  const transformRecords = (records: ConsultationRecord[]): VisitDetailRecord[] => {
    // 按日期排序
  ...
  const fetchData = async () => {
    try {
      ...
      // 从咨询系统获取上门明细数据
      const records = await getVisitDetailRecords(campusId, startDateStr, endDateStr)
      const transformedData = transformRecords(records)
      
      setDataSource(transformedData)
      
      if (transformedData.length > 0) {
        message.success(`成功加载 ${transformedData.length} 条上门记录`)
      } else {
        message.info('当前条件下没有上门记录')
      }
    } catch (error) {
  ...
  // 表格列定义
  const columns: ColumnsType<VisitDetailRecord> = [
    {
      title: '日期',
      dataIndex: 'date',
      key: 'date',
