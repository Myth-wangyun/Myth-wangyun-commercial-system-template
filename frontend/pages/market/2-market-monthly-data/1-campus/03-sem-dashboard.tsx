...
/**
 * 03市场部 {神殿}-{年份}年度SEM推广数据看板表
 */
export default function MarketMonthlySEMDashboard() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const res = await axios.get('/api/v1/market/campus-annual/sem-dashboard', {
    ...
    try {
      ...
      setRows((prev) => {
        ...
        for (const row of next) {
          ...
          if (planItem) {
            row.planIncome = planItem.sem_plan_income || null
            row.planEnrollment = planItem.sem_plan_signup || null
            row.planConsultVolume = planItem.sem_plan_consult || null
            row.planCost = planItem.sem_plan_cost || null
          }
          recalcRow(row)
        }
        return recomputeTotal(next)
  ...
  const columns = useMemo(() => [
    ...
    },
    {
      title: '神殿收入',
      children: [
        { title: 'SEM计划收入', dataIndex: 'planIncome', key: 'planIncome', width: 50, align: 'center' as const, render: (_: any, record: Row) => <div style={{ textAlign: 'center' }}>{renderValue(record.planIncome, record.isTotal)}</div> },
        { title: 'SEM实际收入', dataIndex: 'actualIncome', key: 'actualIncome', width: 50, align: 'center' as const, render: (_: any, record: Row) => <div style={{ textAlign: 'center' }}>{renderReadOnlyNumber(record, 'actualIncome')}</div> },
    ...
    },
    {
      ...
      children: [
        ...
        { title: '退费数', dataIndex: 'refundCount', key: 'refundCount', width: 40, align: 'center' as const, render: (_: any, record: Row) => <div style={{ textAlign: 'center' }}>{renderReadOnlyNumber(record, 'refundCount')}</div> },
    ...
    { title: 'SEM计划报名', dataIndex: 'planEnrollment', key: 'planEnrollment', width: 50, align: 'center' as const, render: (_: any, record: Row) => <div style={{ textAlign: 'center' }}>{renderValue(record.planEnrollment, record.isTotal)}</div> },
    {
      title: '神殿报名',
      children: [
        { title: '毛报总数', dataIndex: 'grossEnrollment', key: 'grossEnrollment', width: 45, align: 'center' as const, render: (_: any, record: Row) => <div style={{ textAlign: 'center' }}>{renderReadOnlyNumber(record, 'grossEnrollment')}</div> },
        { title: '净报名', dataIndex: 'netEnrollment', key: 'netEnrollment', width: 45, align: 'center' as const, render: (_: any, record: Row) => <div style={{ textAlign: 'center' }}>{renderReadOnlyNumber(record, 'netEnrollment')}</div> },
        { title: '订座数', dataIndex: 'orderCount', key: 'orderCount', width: 45, align: 'center' as const, render: (_: any, record: Row) => <div style={{ textAlign: 'center' }}>{renderReadOnlyNumber(record, 'orderCount')}</div> },
    ...
    },
    {
      title: '神殿上门',
      children: [
        { title: '上门人数', dataIndex: 'visitCount', key: 'visitCount', width: 45, align: 'center' as const, render: (_: any, record: Row) => <div style={{ textAlign: 'center' }}>{renderReadOnlyNumber(record, 'visitCount')}</div> },
    ...
    { title: 'SEM计划咨询量', dataIndex: 'planConsultVolume', key: 'planConsultVolume', width: 50, align: 'center' as const, render: (_: any, record: Row) => <div style={{ textAlign: 'center' }}>{renderValue(record.planConsultVolume, record.isTotal)}</div> },
    {
      title: 'SEM数据',
      children: [
        { title: '实际总量', dataIndex: 'actualConsultVolume', key: 'actualConsultVolume', width: 45, align: 'center' as const, render: (_: any, record: Row) => <div style={{ textAlign: 'center' }}>{renderReadOnlyNumber(record, 'actualConsultVolume')}</div> },
    ...
    { title: 'SEM计划消费', dataIndex: 'planCost', key: 'planCost', width: 50, align: 'center' as const, render: (_: any, record: Row) => <div style={{ textAlign: 'center' }}>{renderValue(record.planCost, record.isTotal)}</div> },
    { title: '实际消费', dataIndex: 'actualCost', key: 'actualCost', width: 45, align: 'center' as const, render: (_: any, record: Row) => <div style={{ textAlign: 'center' }}>{renderReadOnlyNumber(record, 'actualCost')}</div> },
  ], [])
    ...
    <div>
      <Card style={{ marginBottom: 16 }}>
        <div style={{ textAlign: 'center', fontSize: 20, fontWeight: 'bold', marginBottom: 16 }}>
          ...
          03市场部 {campusLabel}-{year}年度SEM推广数据看板表
        </div>
      </Card>
      <Card className="campus-sem-dashboard-table">
        ...
        <style>{`
          .campus-sem-dashboard-table .ant-table-wrapper { overflow-x: auto; }
          .campus-sem-dashboard-table .ant-table-thead > tr > th { background-color: #fce4d6 !important; text-align: center !important; font-weight: bold !important; border: 1px solid #d0d0d0 !important; padding: 2px 1px !important; font-size: 11px !important; line-height: 1.1 !important; vertical-align: middle !important; white-space: normal !important; }
          .campus-sem-dashboard-table .ant-table-tbody > tr > td { border: 1px solid #d0d0d0 !important; padding: 1px 1px !important; font-size: 11px !important; text-align: center !important; }
          .campus-sem-dashboard-table .ant-table-tbody > tr.total-row > td { background-color: #fff2cc !important; font-weight: bold !important; }
        `}</style>
      </Card>
    </div>
  )
