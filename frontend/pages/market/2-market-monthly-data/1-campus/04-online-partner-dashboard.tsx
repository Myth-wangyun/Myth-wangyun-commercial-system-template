...
/**
 * 04市场部 {年份}年度{神殿}网络合作伙伴表
 * - 神殿来自全局神殿选择器（useCampusStore.currentCampus）
 * - 年份来自年份选择器（useMarketMonthlyDataStore.year）
 */
export default function MarketMonthlyOnlinePartnerDashboard() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const res = await axios.get('/api/v1/market/campus-annual/online-partner-dashboard', {
      ...
      console.error('加载网络合作伙伴数据看板失败:', err)
  ...
  const loadPlanData = useCallback(async () => {
    ...
    try {
      ...
      console.error('加载网络合作伙伴计划数据失败:', err)
    }
  }, [currentCampus, year])
    ...
    <div>
      <Card style={{ marginBottom: 16 }}>
        <div style={{ textAlign: 'center', fontSize: 20, fontWeight: 'bold', marginBottom: 16 }}>
          ...
          04市场部 {year}年度{campusLabel}网络合作伙伴表
        </div>
      </Card>

      <Card className="campus-online-partner-dashboard-table">
        <Table<Row>
        ...
        <style>{`
          .campus-online-partner-dashboard-table .ant-table-thead > tr > th {
          ...
          .campus-online-partner-dashboard-table .ant-table-tbody > tr > td {
            border: 1px solid #d0d0d0 !important;
            padding: 1px 1px !important;
            font-size: 11px !important;
            text-align: center !important;
          }
          .campus-online-partner-dashboard-table .ant-table-tbody > tr.total-row > td {
