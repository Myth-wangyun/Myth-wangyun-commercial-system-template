...
export default function MgmtMarketReputationDashboard() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      setRows((prev) => {
        ...
        for (const row of next) {
          ...
          if (apiData[monthNum]) {
            ...
            // 实际消费：从 reputation-dashboard API 获取（后端已从月度计划表加载）
      ...
      console.error('加载市场口碑数据看板失败:', err)
      message.error('加载数据失败')
    }
  }, [currentCampus, year])

  // 从月度计划API加载计划数据（实际消费已从 reputation-dashboard API 获取）
    ...
    try {
      ...
      setRows((prev) => {
        ...
        for (const row of next) {
          if (row.isTotal) continue
          const monthNum = parseInt(row.month)
          // 只清空计划字段，不影响 actualCost（它从 reputation-dashboard API 获取）
          row.planIncome = null
      ...
      console.error('加载市场口碑计划数据失败:', err)
    ...
    <div>
      <Card style={{ marginBottom: 16 }}>
        <div style={{ textAlign: 'center', fontSize: 20, fontWeight: 'bold', marginBottom: 16 }}>
          ...
          05最高议事厅 {year}年度{campusLabel}市场口碑表
        </div>
      </Card>

      <Card className="campus-reputation-dashboard-table">
        <Table<Row>
        ...
        <style>{`
          .campus-reputation-dashboard-table .ant-table-thead > tr > th {
          ...
          .campus-reputation-dashboard-table .ant-table-tbody > tr > td {
            border: 1px solid #d0d0d0 !important;
            padding: 1px 1px !important;
            font-size: 11px !important;
            text-align: center !important;
          }
          .campus-reputation-dashboard-table .ant-table-tbody > tr.total-row > td {
