...
interface GroupAChatRateTabProps {
  year: string
  month: string
}

const GroupAChatRateTab: React.FC<GroupAChatRateTabProps> = ({ year, month }) => {
  ...
  const syncChatOutputFromConsultation = async () => {
    try {
      ...
      // 获取聊出量统计数据
      console.log('[A组同步] 开始获取聊出量, 年月:', year, month)
      const chatOutputMap = await getDailyChatOutputByConsultant(year, month)
      console.log('[A组同步] 获取到聊出量映射, 日期数量:', chatOutputMap.size)
      ...
      setDataSource(prev => {
        ...
        newData.forEach((record, index) => {
          ...
          // 根据日期和员工名称查找匹配的聊出量
          console.log(`[A组同步] 尝试匹配: 日期=${record.dateKey}, 员工=${record.employeeName}`)
          const dateStats = chatOutputMap.get(record.dateKey)
          if (dateStats) {
            matchedCount++
            const chatOutput = dateStats.get(record.employeeName)
            console.log(`[A组同步] 日期匹配成功, 查找员工=${record.employeeName}, 结果=${chatOutput}`)
            if (chatOutput !== undefined) {
              if (chatOutput !== record.chatOutput) {
                newData[index] = { ...record, chatOutput }
                updatedCount++
                console.log(`[A组同步] 更新聊出量: ${record.dateKey} - ${record.employeeName}: ${record.chatOutput} -> ${chatOutput}`)
              }
            }
          }
        })
        
        console.log(`[A组同步] 日期匹配成功数: ${matchedCount}, 更新记录数: ${updatedCount}`)
  ...
  const loadEmployeeConfig = async () => {
    try {
      const response = await fetch(
        `/api/v1/market/network-consultant/group-a/config/${year}/${month}`
      )
  ...
  const loadDataFromDatabase = async () => {
    try {
      ...
      // 调用API获取数据
      const response = await fetch(
        `/api/v1/market/network-consultant/group-a/month/${year}/${month}`
      )
  ...
  const handleEditSave = async () => {
    try {
      ...
      // 调用API保存单条数据到数据库
      const response = await fetch('/api/v1/market/network-consultant/group-a/batch', {
        method: 'POST',
        headers: { 
  ...
  const handleSave = async () => {
    try {
      ...
      message.success(result.message || 'A组数据保存成功！')
...
export default GroupAChatRateTab
