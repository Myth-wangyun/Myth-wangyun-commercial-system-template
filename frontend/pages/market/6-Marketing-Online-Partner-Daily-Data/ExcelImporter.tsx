...
export interface ImportedData {
  ...
  baijiao?: PartnerDailyData[]
  zhiliao?: PartnerDailyData[]
  tantu?: PartnerDailyData[]
  houxue?: PartnerDailyData[]
  jiuyisouke?: PartnerDailyData[]
}
...
const ExcelImporter: React.FC<ExcelImporterProps> = ({ campusId, onImportSuccess }) => {
  ...
  // 解析合作伙伴sheet
  const parsePartnerSheet = (worksheet: XLSX.WorkSheet, month: string): PartnerDailyData[] => {
    const data: PartnerDailyData[] = []
    ...
    for (let row = 2; row <= range.e.r; row++) {
      ...
      const rowData: PartnerDailyData = {
        date,
        partnerActualIncome: parseNumber(worksheet[XLSX.utils.encode_cell({ r: row, c: 2 })]?.v), // C列：合作伙伴实际收入
        ...
        actualConsultCount: parseNumber(worksheet[XLSX.utils.encode_cell({ r: row, c: 9 })]?.v), // J列：实际总咨询量
        partnerCost: parseNumber(worksheet[XLSX.utils.encode_cell({ r: row, c: 11 })]?.v), // L列：合作伙伴消费
      }
  ...
  const handleFileUpload = async (file: File) => {
    ...
    try {
      ...
      workbook.SheetNames.forEach((sheetName) => {
        ...
        // 根据sheet名称识别合作伙伴
        if (sheetName.includes('百教') || sheetName.includes('baijiao') || sheetName.includes('02')) {
          const data = parsePartnerSheet(worksheet, month)
          console.log('百教网数据:', data.length, '条')
          if (data.length > 0) {
            console.log('百教网第一条数据日期:', data[0].date)
            if (!actualMonth) {
              actualMonth = dayjs(data[0].date).format('YYYY-MM')
            }
          }
          importedData.baijiao = data
        } else if (sheetName.includes('知了') || sheetName.includes('zhiliao') || sheetName.includes('03')) {
          const data = parsePartnerSheet(worksheet, month)
          console.log('知了好学数据:', data.length, '条')
          if (data.length > 0 && !actualMonth) {
            actualMonth = dayjs(data[0].date).format('YYYY-MM')
          }
          importedData.zhiliao = data
        } else if (sheetName.includes('坦途') || sheetName.includes('tantu') || sheetName.includes('04')) {
          const data = parsePartnerSheet(worksheet, month)
          console.log('坦途网数据:', data.length, '条')
          if (data.length > 0 && !actualMonth) {
            actualMonth = dayjs(data[0].date).format('YYYY-MM')
          }
          importedData.tantu = data
        } else if (sheetName.includes('厚学') || sheetName.includes('houxue') || sheetName.includes('05')) {
          const data = parsePartnerSheet(worksheet, month)
          console.log('厚学网数据:', data.length, '条')
          if (data.length > 0 && !actualMonth) {
            actualMonth = dayjs(data[0].date).format('YYYY-MM')
          }
          importedData.houxue = data
        } else if (sheetName.includes('91') || sheetName.includes('搜客') || sheetName.includes('jiuyisouke') || sheetName.includes('06')) {
          const data = parsePartnerSheet(worksheet, month)
          console.log('91搜客数据:', data.length, '条')
          if (data.length > 0 && !actualMonth) {
            actualMonth = dayjs(data[0].date).format('YYYY-MM')
          }
          importedData.jiuyisouke = data
        }
      })
      ...
      // 显示导入预览
      const summary = []
      if (importedData.baijiao?.length) summary.push(`百教网: ${importedData.baijiao.length}条`)
      if (importedData.zhiliao?.length) summary.push(`知了好学: ${importedData.zhiliao.length}条`)
      if (importedData.tantu?.length) summary.push(`坦途网: ${importedData.tantu.length}条`)
      if (importedData.houxue?.length) summary.push(`厚学网: ${importedData.houxue.length}条`)
      if (importedData.jiuyisouke?.length) summary.push(`91搜客: ${importedData.jiuyisouke.length}条`)

      console.log('导入数据汇总:', summary)
      console.log('完整的importedData:', importedData)

      if (summary.length === 0) {
        message.warning('未能从Excel文件中识别到有效数据，请检查sheet名称是否包含关键字（百教、知了、坦途、厚学、91、搜客）')
        setUploading(false)
        setFileList([])
        return false
      }
