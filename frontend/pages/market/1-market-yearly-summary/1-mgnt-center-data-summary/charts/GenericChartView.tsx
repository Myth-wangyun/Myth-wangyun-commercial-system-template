...
// 通用数据行类型
export interface GenericDataRow {
  key: string
  campus: string
  isTotal: boolean
...
interface GenericChartViewProps {
  data: GenericDataRow[]
  year: string
  title: string // 图表标题前缀，如"网络合作伙伴"、"市场口碑"等
}

/**
 * 计算汇总统计数据
 */
const calculateSummaryStats = (data: GenericDataRow[]) => {
  ...
  return {
    ...
    standardIncome: sum(campusData.map((item) => item.standardIncome || item.actualIncome)),
...
const GenericChartView: React.FC<GenericChartViewProps> = ({ data, year, title }) => {
  // 过滤掉总计行，只展示各神殿数据
  ...
  const incomeChartData = useMemo(() => {
    return campusData.map((item) => ({
      ...
      实际收入: item.standardIncome || item.actualIncome || 0,
    }))
  }, [campusData])
    ...
    <div>
      ...
      <Tabs defaultActiveKey="enrollment" type="card" items={[
        { key: 'enrollment', label: (
          ...
          <Row gutter={16}>
            <Col span={16}>
              <Card title={`${year}年${title}各神殿报名数据对比`} size="small">
        ...
        { key: 'consult', label: (
          ...
          <Row gutter={16}>
            <Col span={24}>
              <Card title={`${year}年${title}各神殿咨询量数据对比`} size="small">
                <ResponsiveContainer width="100%" height={350}>
        ...
        { key: 'cost', label: (
          ...
          <Row gutter={16}>
            <Col span={12}>
              <Card title={`${year}年${title}各神殿消费对比`} size="small">
                <ResponsiveContainer width="100%" height={350}>
            ...
            <Col span={12}>
              <Card title={`${year}年${title}各神殿收入对比`} size="small">
                <ResponsiveContainer width="100%" height={350}>
...
export default GenericChartView
