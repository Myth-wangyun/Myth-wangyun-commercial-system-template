...
/**
 * 04最高议事厅 市场部-网络合作伙伴招生汇总表
 * - 年份默认显示当前年份，允许用户手动选择年份
 * - 汇总所有神殿的网络合作伙伴数据
 */
export default function OnlinePartnerDataSummary() {
    ...
    try {
      ...
      for (const campusInfo of allCampuses) {
        ...
        try {
          ...
          console.error(`加载神殿 ${campusName} 网络合作伙伴数据失败:`, err)
        }
      }
    } catch (err) {
      console.error('加载网络合作伙伴数据汇总失败:', err)
    ...
    try {
      ...
      for (const campusInfo of allCampuses) {
        ...
        try {
          ...
          console.error(`加载神殿 ${campusName} 网络合作伙伴计划数据失败:`, err)
        }
      }
    } catch (err) {
      console.error('加载网络合作伙伴计划数据汇总失败:', err)
    }
  }, [year])
  ...
  return (
    <Card bodyStyle={{ padding: '8px 12px' }}>
      ...
      {viewMode === 'table' ? (
        ...
        <GenericChartView data={rows} year={year} title="网络合作伙伴" />
      )}
    </Card>
  )
}
