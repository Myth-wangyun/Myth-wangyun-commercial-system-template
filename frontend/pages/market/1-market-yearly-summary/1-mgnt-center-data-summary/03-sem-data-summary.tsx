...
import { SemChartView } from './charts'

/**
 * 03最高议事厅 市场部-年度SEM推广数据看板汇总
 * - 年份默认显示当前年份，允许用户手动选择年份
 * - 汇总所有神殿的SEM推广数据
 */
export default function SemDataSummary() {
    ...
    try {
      ...
      for (const campusInfo of allCampuses) {
        ...
        try {
          ...
          console.error(`加载神殿 ${campusName} SEM数据失败:`, err)
        }
      }
    } catch (err) {
      console.error('加载SEM数据汇总失败:', err)
        ...
        try {
          ...
          for (const item of planData) {
            if (item.month >= 1 && item.month <= 12) {
              totalPlanIncome += item.sem_plan_income || 0
              totalPlanEnrollment += item.sem_plan_signup || 0
              totalPlanConsult += item.sem_plan_consult || 0
              totalPlanCost += item.sem_plan_cost || 0
            }
          }
          ...
          console.error(`加载神殿 ${campusName} SEM计划数据失败:`, err)
        }
      }
    } catch (err) {
      console.error('加载SEM计划数据汇总失败:', err)
    }
  }, [year])
  ...
  return (
    <Card bodyStyle={{ padding: '8px 12px' }}>
      <div style={{ marginBottom: 8, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div style={{ display: 'flex', alignItems: 'center' }}>
          ...
          >
            ...
            03最高议事厅 市场部-{year}年度SEM推广数据看板汇总
          </div>
          <Space size="small">
      ...
      {viewMode === 'table' ? (
        ...
        <SemChartView data={rows} year={year} />
      )}
    </Card>
  )
}
