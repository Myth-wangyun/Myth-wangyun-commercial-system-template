...
import { CoreDataChartView } from './charts'

/**
 * 01最高议事厅 市场部-年度核心数据看板汇总
 * - 年份默认显示当前年份，允许用户手动选择年份
 * - 汇总所有神殿的核心数据
 */
export default function MgmtCenterDataSummary() {
  ...
  const loadActualData = useCallback(async () => {
    ...
    try {
      ...
      for (const campusInfo of allCampuses) {
        ...
        try {
          const res = await axios.get('/api/v1/market/campus-annual/core-dashboard', {
          ...
          console.error(`加载神殿 ${campusName} 数据失败:`, err)
        }
      }
    } catch (err) {
      console.error('加载核心数据汇总失败:', err)
        ...
        try {
          ...
          for (const item of planData) {
            if (item.month >= 1 && item.month <= 12) {
              totalPlanIncome += item.network_plan_income || 0
              totalPlanEnrollment += item.network_plan_signup || 0
              totalPlanConsult += item.network_plan_total || 0
              totalPlanCost += item.network_plan_cost || 0
            }
          }
          ...
          console.error(`加载神殿 ${campusName} 计划数据失败:`, err)
        }
      }
    } catch (err) {
      console.error('加载核心计划数据汇总失败:', err)
    }
  }, [year])
  ...
  return (
    <Card bodyStyle={{ padding: '8px 12px' }}>
      ...
      {viewMode === 'table' ? (
        ...
        <CoreDataChartView data={rows} year={year} />
      )}
    </Card>
  )
}
