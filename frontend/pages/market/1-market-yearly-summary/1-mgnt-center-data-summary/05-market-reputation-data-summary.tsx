 ...
 * - 汇总所有神殿的市场口碑数据
 */
export default function MarketReputationDataSummary() {
  ...
  // 从API加载所有神殿的实际数据（不包括实际消费，实际消费从loadPlanData加载）
    ...
    try {
      ...
      setRows((prev) => {
        ...
        for (const row of next) {
          ...
          // 注意：不清空 actualCost，它从 loadPlanData 加载
        }
        return next
      })
      ...
      for (const campusInfo of allCampuses) {
        ...
        try {
          const res = await axios.get('/api/v1/market/campus-annual/reputation-dashboard', {
            params: { campus: campusName, year }
          })
          const apiData = res.data?.data || {}
          
          let totalIncome = 0, totalRefund = 0, totalGross = 0, totalNet = 0
          let totalOrder = 0, totalVisit = 0, totalConsult = 0
          ...
          for (let month = 1; month <= 12; month++) {
            if (apiData[month]) {
              ...
              // 注意：不从此API加载 actualCost，它从月度计划表的 actual_expense 字段获取
            }
          }
          ...
          setRows((prev) => {
            ...
            if (idx !== -1 && !next[idx].isTotal) {
              ...
              // 注意：actualCost 从 loadPlanData 加载
              recalcRow(next[idx])
            }
            return recomputeTotal(next)
          })
        } catch (err) {
          console.error(`加载神殿 ${campusName} 市场口碑数据失败:`, err)
        }
      }
    } catch (err) {
      console.error('加载市场口碑数据汇总失败:', err)
    ...
    try {
      ...
      for (const campusInfo of allCampuses) {
        ...
        try {
          ...
          let totalPlanIncome = 0, totalPlanEnrollment = 0, totalPlanConsult = 0, totalPlanCost = 0, totalActualCost = 0
          ...
          for (let month = 1; month <= 12; month++) {
            if (planData[month]) {
              ...
              totalActualCost += planData[month].actual_expense || 0
            }
          }
          ...
          setRows((prev) => {
            ...
            if (idx !== -1 && !next[idx].isTotal) {
              ...
              next[idx].actualCost = totalActualCost || null
              recalcRow(next[idx])
            }
            return recomputeTotal(next)
          })
        } catch (err) {
          console.error(`加载神殿 ${campusName} 市场口碑计划数据失败:`, err)
        }
      }
    } catch (err) {
      console.error('加载市场口碑计划数据汇总失败:', err)
    }
  }, [year])
  ...
  return (
    <Card bodyStyle={{ padding: '8px 12px' }}>
      ...
      {viewMode === 'table' ? (
        ...
        <GenericChartView data={rows} year={year} title="市场口碑" />
      )}
    </Card>
  )
}
