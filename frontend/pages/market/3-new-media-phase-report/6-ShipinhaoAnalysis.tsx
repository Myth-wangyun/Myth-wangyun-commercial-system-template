...
interface ShipinhaoAnalysisData {
  key: string
  date: string
  category: string
  campus: string
...
const ShipinhaoAnalysisPage: React.FC = () => {
  ...
  const [analysisData, setAnalysisData] = useState<ShipinhaoAnalysisData[]>([])
  ...
  // 神殿列表（显示名称）
  const campusList = ['盛邦', '冀美', '晋美', '太美', '原美', '桂美', '黔美']
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const dataPromises = campusList.map(async (campusDisplayName) => {
        ...
        try {
          ...
          for (const month of months) {
            const response = await wechatVideoDailyDataService.list(campusDbName, month)
            console.log(`[视频号] ${campusDbName} ${month} 响应:`, response)
            const items = response.data?.items || (response as any).items || []
            console.log(`[视频号] ${campusDbName} ${month} items:`, items)
            allItems.push(...items)
          }
      ...
      // 构建表格数据
      const formatData = (data: any, campus: string, key: string): ShipinhaoAnalysisData => {
        ...
        const cpm = data.exposure > 0 
          ? ((data.actualSales / data.exposure) * 1000).toFixed(2)
          : '0'
        
        const clickRate = data.exposure > 0
          ? ((data.clicks / data.exposure) * 100).toFixed(2) + '%'
          : '0%'
      ...
      console.log('[视频号] 最终表格数据:', tableData)
  ...
  const columns = [
    {
      ...
      width: 70,
      fixed: 'left' as const,
      render: (text: string, record: ShipinhaoAnalysisData, index: number) => {
    ...
    // 销售数据
    {
      ...
      key: 'plannedSales',
      width: 120,
      render: (text: number, record: ShipinhaoAnalysisData) => {
      ...
      <Card>
        ...
        <Row gutter={[16, 16]} style={{ marginBottom: '24px' }}>
          <Col xs={24} sm={6}>
            ...
            >
              <div style={{ fontSize: '12px', marginBottom: '8px' }}>曝光次数</div>
              <div style={{ fontSize: '24px', fontWeight: 'bold' }}>{analysisData.find(d => d.key === 'total')?.exposure || 0}</div>
            </Card>
          </Col>
          <Col xs={24} sm={6}>
            <Card
            ...
            >
              <div style={{ fontSize: '12px', marginBottom: '8px' }}>点击次数</div>
              <div style={{ fontSize: '24px', fontWeight: 'bold' }}>{analysisData.find(d => d.key === 'total')?.clicks || 0}</div>
            </Card>
          </Col>
          <Col xs={24} sm={6}>
            <Card
            ...
            >
              ...
              <div style={{ fontSize: '24px', fontWeight: 'bold' }}>{analysisData.find(d => d.key === 'total')?.actualConsultation || 0}</div>
            </Card>
          </Col>
          <Col xs={24} sm={6}>
            <Card
            ...
            >
              ...
              <div style={{ fontSize: '24px', fontWeight: 'bold' }}>{analysisData.find(d => d.key === 'total')?.actualIncome || 0}</div>
            </Card>
          </Col>
        </Row>
        ...
        {/* 指标说明 */}
        <Card type="inner" title="视频号数据指标说明" style={{ marginTop: '24px' }}>
          <Row gutter={[16, 16]}>
            <Col xs={24} sm={12}>
              <div style={{ fontSize: '12px', lineHeight: '1.8' }}>
                <div style={{ fontWeight: 'bold', marginBottom: '8px', color: '#1890ff' }}>曝光与点击</div>
                <ul style={{ marginBottom: '0', paddingLeft: '20px' }}>
                  <li>曝光次数：广告在视频号平台的展示次数</li>
                  <li>千次展现均价（CPM）：每千次曝光的成本</li>
                  <li>点击次数：用户点击广告的次数</li>
                  <li>点击率：点击次数 / 曝光次数 × 100%</li>
                  <li>点击均价：每次点击的平均成本</li>
                </ul>
              </div>
            </Col>
            <Col xs={24} sm={12}>
...
export default ShipinhaoAnalysisPage
