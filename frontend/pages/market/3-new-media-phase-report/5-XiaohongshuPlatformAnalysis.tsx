...
interface XiaohongshuAnalysisData {
  key: string
  date: string
  category: string
  campus: string
...
const XiaohongshuPlatformAnalysisPage: React.FC = () => {
    ...
    try {
      ...
      const dataPromises = campusList.map(async (campusDisplayName) => {
        ...
        try {
          ...
          console.log(`[${campusDisplayName}] API响应:`, { campusDbName, months, itemsCount: allItems.length, allItems })
          ...
          console.log(`[${campusDisplayName}] 过滤后数据:`, filteredItems.length)

          // 聚合数据 - 使用parseFloat确保数值正确解析
          const aggregated = filteredItems.reduce((acc, item) => ({
            ...
            retention: acc.retention + (parseFloat(item.private_message_count as any) || 0),
      ...
      // 构建表格数据
      const formatData = (data: any, campus: string, key: string): XiaohongshuAnalysisData => {
  ...
  const columns = [
    {
      ...
      width: 70,
      fixed: 'left' as const,
      render: (text: string, record: XiaohongshuAnalysisData, index: number) => {
    ...
    // 销售数据
    {
      ...
      key: 'plannedSales',
      width: 120,
      render: (text: number, record: XiaohongshuAnalysisData) => {
        ...
        <Row gutter={[16, 16]} style={{ marginBottom: '24px' }}>
          <Col xs={24} sm={6}>
            ...
            >
              ...
              <div style={{ fontSize: '24px', fontWeight: 'bold' }}>{analysisData.find(d => d.key === 'total')?.impressions || 0}</div>
            </Card>
          </Col>
          <Col xs={24} sm={6}>
            <Card
          ...
          <Col xs={24} sm={6}>
            ...
            >
              <div style={{ fontSize: '12px', marginBottom: '8px' }}>私信留资数</div>
              <div style={{ fontSize: '24px', fontWeight: 'bold' }}>{analysisData.find(d => d.key === 'total')?.retention || 0}</div>
            </Card>
          </Col>
          <Col xs={24} sm={6}>
            <Card
...
export default XiaohongshuPlatformAnalysisPage
