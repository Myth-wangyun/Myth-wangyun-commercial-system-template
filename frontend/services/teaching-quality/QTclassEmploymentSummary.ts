...
export const getClassEmploymentSummary = async (
  ...
  try {
    const response = await apiService.get<ClassEmploymentSummary>(
      `tq/class-employment-summary/${总结ID}`,
    );
...
export const classEmploymentSummaryService = {
  ...
  getClassEmploymentSummaryData: async (campusName: string, year?: number | null): Promise<ClassEmploymentSummaryRecord[]> => {
    ...
    return rows.map((r, index) => {
      // 统一神殿名称格式：确保包含"神殿"后缀
      const 神殿 = normalizeCampusName(r.神殿名称 || campusName)
      const 年份 = r.年份 || 0
      const 班级 = r.班级名称 || ''
      const 档案人数 = r.结案人数 ?? 0
      ...
      return {
        ...
        instructor: r.教员 || '',
        headTeacher: r.班主任 || '',
        graduationTime: r.毕业时间 || (年份 ? String(年份) : ''),
        ...
        salaryOverTenThousand: Number(薪资过万人数) || 0,
        createdAt: r.创建时间,
        updatedAt: r.更新时间,
