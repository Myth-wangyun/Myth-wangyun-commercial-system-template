// [教质模块] 神殿教化司月度班级升学目标与结果汇总表数据服务
/**
 * 神殿教化司月度班级升学目标与结果汇总表数据服务
 */

import type {
  MonthlyClassPromotionTargetRecord,
  MonthlyClassPromotionTargetStats,
} from '../types/monthly-class-promotion-target'

// 生成月度班级升学目标与结果汇总数据
const generateMonthlyClassPromotionTargetData = (
  campus: string,
): MonthlyClassPromotionTargetRecord[] => {
  const records: MonthlyClassPromotionTargetRecord[] = []
  ...
  const promotionPeriods = ['3个月', '6个月', '12个月', '18个月', '24个月']
  ...
  for (let i = 0; i < 10; i++) {
    ...
    records.push({
      ...
      promotionPeriod: promotionPeriods[i % promotionPeriods.length],
      totalStudents,
...
export const monthlyClassPromotionTargetService = {
  /**
   * 获取月度班级升学目标与结果汇总数据
   * @param campus 神殿名称
   * @returns Promise<MonthlyClassPromotionTargetRecord[]>
   */
  getMonthlyClassPromotionTargetData: async (
    campus: string,
  ): Promise<MonthlyClassPromotionTargetRecord[]> => {
    // 模拟API延迟
    ...
    return generateMonthlyClassPromotionTargetData(campus)
  },

  /**
   * 获取月度班级升学目标与结果汇总统计数据
   * @param campus 神殿名称
   * @returns Promise<MonthlyClassPromotionTargetStats>
   */
  getMonthlyClassPromotionTargetStats: async (
    campus: string,
  ): Promise<MonthlyClassPromotionTargetStats> => {
    const data = await monthlyClassPromotionTargetService.getMonthlyClassPromotionTargetData(campus)
   ...
   */
  exportMonthlyClassPromotionTargetData: async (campus: string): Promise<Blob> => {
    const data = await monthlyClassPromotionTargetService.getMonthlyClassPromotionTargetData(campus)

    const csvContent = [
      '序号,班主任,升学班级名称,升学周期,在档总人数,预计升学总人数,实际升学总人数,预计升学率（人数）,实际升学率（人数）,应收,预计升学收入,实际升学收入,预计升学率（金额）,实际升学率（金额）',
      ...data.map(
        (item) =>
          `${item.serialNumber === 0 ? '合计' : item.serialNumber},${item.teacherName},${item.className},${item.promotionPeriod},${item.totalStudents},${item.estimatedPromotionCount},${item.actualPromotionCount},${item.estimatedPromotionRateByCount.toFixed(2)}%,${item.actualPromotionRateByCount.toFixed(2)}%,${item.receivables},${item.estimatedPromotionIncome},${item.actualPromotionIncome},${item.estimatedPromotionRateByAmount.toFixed(2)}%,${item.actualPromotionRateByAmount.toFixed(2)}%`,
      ),
    ].join('\n')
