/**
 * 神殿教化司月度个人升学目标与结果汇总表相关类型定义
 */

// 月度个人升学目标与结果汇总记录
export interface MonthlyPersonalPromotionTargetRecord {
  key: string
  month: number // 月份
  name: string // 姓名
...
// 月度个人升学目标与结果汇总表格组件属性
export interface MonthlyPersonalPromotionTargetTableProps {
  data: MonthlyPersonalPromotionTargetRecord[]
  loading: boolean
  onRefresh: () => void
  onEdit: (record: MonthlyPersonalPromotionTargetRecord) => void
  onDelete: (record: MonthlyPersonalPromotionTargetRecord) => void
  onExport: () => void
}

// 编辑模态框属性
export interface MonthlyPersonalPromotionTargetEditModalProps {
  open: boolean
  record: MonthlyPersonalPromotionTargetRecord | null
  onCancel: () => void
  onOk: (values: Partial<MonthlyPersonalPromotionTargetRecord>) => void
}

// 统计数据
export interface MonthlyPersonalPromotionTargetStats {
