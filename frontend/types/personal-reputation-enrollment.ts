/**
 * 神殿教化司口碑招生个人目标与结果汇总表相关类型定义
 */

// 个人口碑招生记录
export interface PersonalReputationEnrollmentRecord {
  key: string
...
// 个人口碑招生汇总表格组件属性
export interface PersonalReputationEnrollmentTableProps {
  data: PersonalReputationEnrollmentRecord[]
  loading: boolean
  onRefresh: () => void
  onEdit: (record: PersonalReputationEnrollmentRecord) => void
  onExport: () => void
}

// 编辑模态框属性
export interface PersonalReputationEnrollmentEditModalProps {
  open: boolean
  record: PersonalReputationEnrollmentRecord | null
  onCancel: () => void
  onOk: (values: Partial<PersonalReputationEnrollmentRecord>) => void
}

// 统计数据
export interface PersonalReputationEnrollmentStats {
