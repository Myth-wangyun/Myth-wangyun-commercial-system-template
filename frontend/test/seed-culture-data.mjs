 ...
 * 
 * 功能：
 * - 向至少2个校区、2个年份、2个月份各插入10条数据
 * - 测试企业文化宣讲计划表
 * - 测试企业文化考试计划表
 * 
 * 使用方法：
 * node frontend/test/seed-culture-data.mjs
 * 或
...
// 生成宣讲计划测试数据
function generatePresentationPlanRows(count, year, month) {
  const rows = []
...
// 生成考试计划测试数据
function generateExamPlanRows(count, year, month) {
  const rows = []
...
// 保存宣讲计划
async function savePresentationPlan(campus, year, month, rows) {
...
// 保存考试计划
async function saveExamPlan(campus, year, month, rows) {
  const 行数据 = rows.map((row) => ({
    序号: row.index,
...
async function runTest() {
  ...
  console.log('='.repeat(60))
  console.log(`配置信息:`)
  console.log(`  校区: ${TEST_CONFIG.campuses.join(', ')}`)
  ...
  for (const campus of TEST_CONFIG.campuses) {
    ...
    for (const year of TEST_CONFIG.years) {
      ...
      for (const month of TEST_CONFIG.months) {
        ...
        const presentationSuccess = await savePresentationPlan(campus, year, month, presentationRows)
        ...
        const examSuccess = await saveExamPlan(campus, year, month, examRows)
        if (examSuccess) {
          stats.exam.success++
        } else {
          stats.exam.failed++
  ...
  return {
    success: stats.presentation.failed === 0 && stats.exam.failed === 0,
    stats,
  }
}
